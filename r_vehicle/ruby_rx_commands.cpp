...
bool process_command(u8* pBuffer, int length)
{
   ...
   if ( uCommandType == COMMAND_ID_GET_CPU_INFO )
   {
      ...
      sprintf(szOutput, "Avg/Max loops (ms): rx_commands: %u/%u;", s_pProcessStats->uAverageLoopTimeMs, s_pProcessStats->uMaxLoopTimeMs);
      strcat(szBuffer, szOutput);
   ...
   {
      int linkIndex = (pPHC->command_param)>>16;
      int newFreq = (pPHC->command_param) & 0xFFFF;
      log_line("Received command to change frequency on radio link %d to %d Mhz", linkIndex+1, newFreq);

      bool freqIsOk = false;
      if ( linkIndex >= 0 && linkIndex < g_pCurrentModel->radioLinksParams.links_count )
      {
         for( int i=0; i<g_pCurrentModel->radioInterfacesParams.interfaces_count; i++ )
            if ( g_pCurrentModel->radioInterfacesParams.interface_link_id[i] == linkIndex )
            if ( hardware_radioindex_supports_frequency(i, newFreq) )
               freqIsOk = true;
      }
      ...
      log_line("Switching vehicle radio link %d to %d Mhz.", linkIndex+1, newFreq);
      for( int i=0; i<g_pCurrentModel->radioInterfacesParams.interfaces_count; i++ )
         if ( g_pCurrentModel->radioInterfacesParams.interface_link_id[i] == linkIndex )
         {
            launch_set_frequency(g_pCurrentModel, i, newFreq);
            g_pCurrentModel->radioInterfacesParams.interface_current_frequency[i] = newFreq;
         }
      hardware_save_radio_info();

      g_pCurrentModel->radioLinksParams.link_frequency[linkIndex] = newFreq;
...
int main(int argc, char *argv[])
{
   ...
   s_pProcessStats = shared_mem_process_stats_open_write(SHARED_MEM_WATCHDOG_COMMANDS_RX);
   if ( NULL == s_pProcessStats )
   ...
   while (!g_bQuit) 
   {
      ...
      while ( (maxMsgToRead > 0) && NULL != ruby_ipc_try_read_message(s_fIPCFromRouter, 40000, s_PipeTmpBufferCommands, &s_PipeTmpBufferCommandsPos, s_BufferCommands) )
      {
         ...
         if ( (pPH->packet_flags & PACKET_FLAGS_MASK_MODULE) == PACKET_COMPONENT_LOCAL_CONTROL )
         {
            ...
            {
               int* pI = (int*)((&s_BufferCommands[0])+sizeof(t_packet_header));
               int nLink = *pI;
               pI++;
               int newFreq = *pI;
               log_line("Received new radio link frequency from router (radio link %d new freq: %d Mhz). Updating local model copy.", nLink+1, newFreq);
   ...
   shared_mem_process_stats_close(SHARED_MEM_WATCHDOG_COMMANDS_RX, s_pProcessStats);
