...
static u32 s_uRadioRxTimeoutCounter = 0;
...
type_uplink_rx_info_stats s_UplinkInfoRxStats[MAX_RADIO_INTERFACES];
...
u32 s_uTimeLastCommandSowftwareUpload = 0;
...
int open_radio_interfaces()
{
   ...
   for( int i=0; i<hardware_get_radio_interfaces_count(); i++ )
   {
      ...
      if ( pNIC->lastFrequencySetFailed )
         continue;
      int iRadioLinkId = g_pCurrentModel->radioInterfacesParams.interface_link_id[i];
      if ( iRadioLinkId < 0 )
      {
   ...
   for( int i=0; i<hardware_get_radio_interfaces_count(); i++ )
   {
      ...
      if ( pNIC->openedForRead && pNIC->openedForWrite )
         log_line(" * Interface %s, %s, %d Mhz opened for read/write, flags: %s", pNIC->szName, pNIC->szDriver, pNIC->currentFrequency, szFlags );
      else if ( pNIC->openedForRead )
         log_line(" * Interface %s, %s, %d Mhz opened for read, flags: %s", pNIC->szName, pNIC->szDriver, pNIC->currentFrequency, szFlags );
      else if ( pNIC->openedForWrite )
         log_line(" * Interface %s, %s, %d Mhz opened for write, flags: %s", pNIC->szName, pNIC->szDriver, pNIC->currentFrequency, szFlags );
      else
         log_line(" * Interface %s, %s, %d Mhz not used. Flags: %s", pNIC->szName, pNIC->szDriver, pNIC->currentFrequency, szFlags );
   }
   log_line("=========================================================");
...
void send_radio_reinitialized_message()
{
   ...
   send_packet_to_radio_interfaces(false, packet, PH.total_length);
}
...
void reinit_radio_interfaces()
{
   ...
   hw_execute_bash_command("rm -rf tmp/ruby/conf_nics", NULL);
   ...
   for( int i=0; i<g_pCurrentModel->radioInterfacesParams.interfaces_count; i++ )
   {
      ...
      launch_set_frequency(g_pCurrentModel, i, g_pCurrentModel->radioInterfacesParams.interface_current_frequency[i] );
   }
...
void _try_decode_controller_links_stats_from_packet(u8* pPacketData, int packetLength)
{
   ...
   if ( pPH->total_length > sizeof(t_packet_header) + sizeof(u32) + sizeof(u8) )
   {
      ...
      pData = pPacketData + sizeof(t_packet_header) + sizeof(u32) + sizeof(u8);
   }
...
int try_receive_radio_packets(u32 uMaxMicroSeconds)
{
   ...
   for( int i=0; i<hardware_get_radio_interfaces_count(); i++ )
   {
      radio_hw_info_t* pNICRX = hardware_get_radio_info(i);
      if ( ! pNICRX->openedForRead )
         continue;
      FD_SET(pNICRX->monitor_interface_read.selectable_fd, &s_ReadSetRXRadio);
   ...
   if ( nResult == 0 || ((NULL != s_pProcessStats) && (0 != s_pProcessStats->lastRadioRxTime) && (s_pProcessStats->lastRadioRxTime < g_TimeNow + 3000)) )
   if ( g_pCurrentModel->clock_sync_type != CLOCK_SYNC_TYPE_NONE )
   {
      if ( (NULL != s_pProcessStats) && (s_pProcessStats->lastRadioRxTime < g_TimeNow - 3000) )
   ...
   if ( (pPH->packet_flags & PACKET_FLAGS_MASK_MODULE) == PACKET_COMPONENT_COMMANDS )
   {
      if ( pPH->packet_type == PACKET_TYPE_COMMAND )
      {
         ...
         if ( pPHC->command_type == COMMAND_ID_UPLOAD_SW_TO_VEHICLE || pPHC->command_type == COMMAND_ID_UPLOAD_SW_TO_VEHICLE63 )
            s_uTimeLastCommandSowftwareUpload = g_TimeNow;
...
int _handle_received_packet_error(int iNICIndex, u8* pData, int nDataLength, int* pCRCOk)
{
   ...
   if ( iRadioError == RADIO_PROCESSING_ERROR_CODE_INVALID_CRC_RECEIVED )
   {
      ...
      int nPacketLength = packet_process_and_check(iNICIndex, pData, nDataLength, pCRCOk);

      if ( nPacketLength > 0 )
      {
         if ( g_TimeNow > s_UplinkInfoRxStats[iNICIndex].timeLastLogWrongRxPacket + 10000 )
         {
            char szBuff[256];
            ...
            s_UplinkInfoRxStats[iNICIndex].timeLastLogWrongRxPacket = g_TimeNow;
...
void _process_received_full_radio_packet(int iNICIndex)
{
   int bufferLength = 0;
   u8* pBuffer = NULL;
   int bCRCOk = 0;

   radio_hw_info_t* pNICInfo = hardware_get_radio_info(iNICIndex);

   #ifdef PROFILE_RX
   u32 timeNow = get_current_timestamp_ms();
   #endif

   pBuffer = radio_process_wlan_data_in(iNICIndex, &bufferLength);

   #ifdef PROFILE_RX
   u32 dTime1 = get_current_timestamp_ms() - timeNow;
   if ( dTime1 >= PROFILE_RX_MAX_TIME )
      log_softerror_and_alarm("[Profile-Rx] Reading received radio packet from radio interface %d took too long: %d ms.", iNICIndex+1, (int)dTime1);
   #endif
   ...
   if ( pBuffer == NULL )
   {
      ...
      if ( radio_get_last_read_error_code() == RADIO_READ_ERROR_TIMEDOUT )
      {
         ...
         s_uRadioRxTimeoutCounter++;
         send_alarm_to_controller(ALARM_ID_VEHICLE_RX_TIMEOUT, 1+(u32)iNICIndex, s_uRadioRxTimeoutCounter);
         return;
      }
   ...
   u32 gap = g_TimeNow - g_TimeLastReceivedRadioPacket;
   if ( gap > 254 )
      gap = 254;
   ...
   g_TimeLastReceivedRadioPacket = g_TimeNow;
   if ( 0 == g_TimeFirstReceivedRadioPacket )
      g_TimeFirstReceivedRadioPacket = g_TimeNow;
   ...
   while ( nLength > 0 )
   {
      ...
      t_packet_header* pPH = (t_packet_header*)pData;
      bCRCOk = false;
      int nPacketLength = packet_process_and_check(iNICIndex, pData, nLength, &bCRCOk);

      #ifdef PROFILE_RX
      u32 dTime2 = get_current_timestamp_ms() - uTime2;
      if ( dTime2 >= PROFILE_RX_MAX_TIME )
         log_softerror_and_alarm("[Profile-Rx] Processing received single radio packet (type: %d, length: %d/%d bytes) from radio interface %d took too long: %d ms.", pPH->packet_type, pPH->total_length, nPacketLength, iNICIndex+1, (int)dTime2);
      #endif

      int nRes = radio_stats_update_on_packet_received(&g_SM_RadioStats, g_TimeNow, iNICIndex, pData, nPacketLength, (int)bCRCOk);
      if ( nRes != 1 )
      {
      ...
      s_UplinkInfoRxStats[iNICIndex].lastReceivedDBM = pNICInfo->monitor_interface_read.radioInfo.nDbm;
      s_UplinkInfoRxStats[iNICIndex].lastReceivedDataRate = pNICInfo->monitor_interface_read.radioInfo.nRate;

      if ( nPacketLength <= 0 )
      {
         nPacketLength = _handle_received_packet_error(iNICIndex, pData, nLength, &bCRCOk);
         if ( nPacketLength <= 0 )
         {
            return;
         }
      ...
      {
         if ( g_TimeNow > s_UplinkInfoRxStats[iNICIndex].timeLastLogWrongRxPacket + 2000 )
         {
            s_UplinkInfoRxStats[iNICIndex].timeLastLogWrongRxPacket = g_TimeNow;
      ...
      _process_received_radio_packet(iNICIndex, pData, pPH->total_length);
      pData += pPH->total_length;
      nLength -= pPH->total_length;
   }
   ...
   if ( dTimeLast >= PROFILE_RX_MAX_TIME )
      log_softerror_and_alarm("[Profile-Rx] Reading and processing received radio packet (%d bytes, %d subpackets) from radio interface %d took too long: %d ms.", bufferLength, iCountPackets, iNICIndex+1, (int)dTimeLast);
   #endif

}

void _process_received_radio_packets()
{
   for( int i=0; i<hardware_get_radio_interfaces_count(); i++ )
   {
      radio_hw_info_t* pNICRX = hardware_get_radio_info(i);
      if ( ! pNICRX->openedForRead )
         continue;
      if( 0 != FD_ISSET(pNICRX->monitor_interface_read.selectable_fd, &s_ReadSetRXRadio) )
   ...
   while ( packets_queue_has_packets(&s_QueueRadioPacketsOut) )
   {
      ...
      if ( (pPH->packet_flags & PACKET_FLAGS_MASK_MODULE) == PACKET_COMPONENT_TELEMETRY )
      {
         ...
         if ( pPH->packet_type == PACKET_TYPE_RUBY_TELEMETRY_EXTENDED )
         {
            ...
            t_packet_header_ruby_telemetry_extended* pPHRTE = (t_packet_header_ruby_telemetry_extended*) (pBuffer + sizeof(t_packet_header));
            pPHRTE->downlink_tx_video_bitrate = process_data_tx_video_get_current_video_bitrate_average() + get_audio_bps();
            pPHRTE->downlink_tx_video_all_bitrate = process_data_tx_video_get_current_total_bitrate_average() + get_audio_bps();
            ...
            for( int i=0; i<hardware_get_radio_interfaces_count(); i++ )
            {
               ...
               pPHRTE->uplink_datarate[i] = s_UplinkInfoRxStats[i].lastReceivedDataRate;
               pPHRTE->uplink_rssi_dbm[i] = s_UplinkInfoRxStats[i].lastReceivedDBM + 200;
            ...
            if ( pPH->total_length == (sizeof(t_packet_header) + sizeof(t_packet_header_ruby_telemetry_extended) + sizeof(t_packet_header_ruby_telemetry_extended_extra_info_retransmissions)) )
            {
                memcpy( pBuffer + sizeof(t_packet_header) + sizeof(t_packet_header_ruby_telemetry_extended), (u8*)&g_PHTE_Retransmissions, sizeof(g_PHTE_Retransmissions));
            }
         }
      }
      ...
      {
         send_packet_to_radio_interfaces(false, composed_packet, composed_packet_length);
...
int periodic_loop()
{
   ...
   if ( s_iPendingFrequencyChangeLinkId >= 0 && s_uPendingFrequencyChangeTo > 100 && s_uTimeFrequencyChangeRequest != 0 && g_TimeNow > s_uTimeFrequencyChangeRequest + VEHICLE_SWITCH_FREQUENCY_AFTER_MS )
   {
      log_line("Processing pending RC trigger to change frequency to: %d Mhz on link: %d", s_uPendingFrequencyChangeTo, s_iPendingFrequencyChangeLinkId+1 );
      ...
      for( int i=0; i<g_pCurrentModel->nic_count; i++ )
      {
         ...
         {
            launch_set_frequency(g_pCurrentModel, i, s_uPendingFrequencyChangeTo); 
      ...
      *pI = s_iPendingFrequencyChangeLinkId;
      pI++;
   ...
   if ( g_TimeNow >= g_SM_DevVideoBitrateHistory.uLastComputeTime + g_SM_DevVideoBitrateHistory.uComputeInterval )
   {
      ...
      for( int i=MAX_INTERVALS_VIDEO_BITRATE_HISTORY-1; i>0; i-- )
      {
         ...
         g_SM_DevVideoBitrateHistory.uHistTotalVideoBitrateKb[i] = g_SM_DevVideoBitrateHistory.uHistTotalVideoBitrateKb[i-1]; 
      }
      ...
      g_SM_DevVideoBitrateHistory.uHistVideoBitrateKb[0] = process_data_tx_video_get_current_video_bitrate_average()/1000;
      g_SM_DevVideoBitrateHistory.uHistTotalVideoBitrateKb[0] = process_data_tx_video_get_current_total_bitrate_average()/1000;
...
void _check_loop_consistency(int iStep, u32 uLastTotalTxPackets, u32 uLastTotalTxBytes, u32 tTime0, u32 tTime1, u32 tTime2, u32 tTime3, u32 tTime4, u32 tTime5)
{
   ...
   if ( g_TimeNow > g_TimeLastSetRadioFlagsCommandReceived + 5000 )
   {
      if ( s_pProcessStats->uMaxLoopTimeMs < tTime5 - tTime0 )
         s_pProcessStats->uMaxLoopTimeMs = tTime5 - tTime0;
      s_pProcessStats->uTotalLoopTime += tTime5 - tTime0;
...
int main (int argc, char *argv[])
{
   ...
   s_pProcessStats = shared_mem_process_stats_open_write(SHARED_MEM_WATCHDOG_ROUTER_TX);
   if ( NULL == s_pProcessStats )
   ...
   {
      shared_mem_process_stats_close(SHARED_MEM_WATCHDOG_ROUTER_TX, s_pProcessStats);
      return -1;
   }
   ...
   for( int i=0; i<MAX_RADIO_INTERFACES; i++ )
   {
      memset(&s_UplinkInfoRxStats[i], 0, sizeof(type_uplink_rx_info_stats));
      s_UplinkInfoRxStats[i].lastReceivedDBM = 200;
      s_UplinkInfoRxStats[i].lastReceivedDataRate = 0;
      s_UplinkInfoRxStats[i].timeLastLogWrongRxPacket = 0;
   }
   ...
   g_SM_DevVideoBitrateHistory.uComputeInterval = 100;
   ...
   u32 uSleepMicroInterval = 500;

   while ( !g_bQuit )
   {
      //hardware_sleep_micros(uSleepMicroInterval);
      ...
      for( int i=0; i<3; i++ )
      {
         readResult = try_receive_radio_packets(uSleepMicroInterval/3);
         if( readResult > 0 )
            _process_received_radio_packets();
      ...
      do
      {
         ...
         {
            uSleepMicroInterval += 400;
            if ( uSleepMicroInterval > 3000 )
               uSleepMicroInterval = 3000;
            break;
         }
         else
            uSleepMicroInterval = 200;
      ...
      if ( 0 != s_uTimeLastCommandSowftwareUpload )
      if ( g_TimeNow < s_uTimeLastCommandSowftwareUpload + 2000 )
         bSendPacketsNow = true;
