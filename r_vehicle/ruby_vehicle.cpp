...
int main (int argc, char *argv[])
{
   ...
   u32 alarms = modelVehicle.alarms;
   ...
   {
       radio_hw_info_t* pNICInfo = hardware_get_radio_info_from_mac(modelVehicle.radioInterfacesParams.interface_szMAC[i]);
       if ( NULL == pNICInfo )
          continue;
       // If user defined, do not change it
       if ( modelVehicle.radioInterfacesParams.interface_card_model[i] < 0 )
       {
          log_line("Radio interface %d model was set by user to [%s]. Do not change it (to [%s]).", i+1, str_get_radio_card_model_string(-modelVehicle.radioInterfacesParams.interface_card_model[i]), str_get_radio_card_model_string(pNICInfo->iCardModel) );
          continue;
       }
       if ( modelVehicle.radioInterfacesParams.interface_card_model[i] != pNICInfo->iCardModel )
       {
          modelVehicle.radioInterfacesParams.interface_card_model[i] = pNICInfo->iCardModel;
          bMustSave = true;
       }
   }
   ...
   log_line("Setting all the radio cards params...");
