...
bool _parse_ltm_message(t_packet_header_fc_telemetry* pdpfct, t_packet_header_ruby_telemetry_extended* pPHRTE, u8 vehicleType)
{
   ...
   if ( s_LTMExpectedFrameType == LIGHTTELEMETRY_GFRAME )
   {
      ...
      {
         s_TimeLastLTM_Altitude = g_TimeNow;
      ...
      else
      {
         ...
         if ( g_TimeNow > s_TimeLastLTM_Altitude )
         {
            long dTime = g_TimeNow - s_TimeLastLTM_Altitude; 
...
bool parse_telemetry_from_fc_ltm( u8* buffer, int length, t_packet_header_fc_telemetry* pphfct, t_packet_header_ruby_telemetry_extended* pPHRTE, u8 vehicleType)
{
   bool ret = false;
   u8 c;
