...
void hardware_log_radio_info()
{
   ...
   {
      log_line("* RadioInterface %d: %s MAC:%s phy#%d, supported: %s, USBPort: %s now at %d Mhz;", i+1, sRadioInfo[i].szName, sRadioInfo[i].szMAC, sRadioInfo[i].phy_index, sRadioInfo[i].isSupported?"yes":"no", sRadioInfo[i].szUSBPort, sRadioInfo[i].currentFrequency);
      ...
      if ( sRadioInfo[i].supportedBands & RADIO_HW_SUPPORTED_BAND_23 )
         strcat( szBuff, "2.3Ghz ");
      if ( sRadioInfo[i].supportedBands & RADIO_HW_SUPPORTED_BAND_24 )
         strcat( szBuff, "2.4Ghz ");
      if ( sRadioInfo[i].supportedBands & RADIO_HW_SUPPORTED_BAND_25 )
         strcat( szBuff, "2.5Ghz ");
      if ( sRadioInfo[i].supportedBands & RADIO_HW_SUPPORTED_BAND_58 )
         strcat( szBuff, "5.8Ghz ");
      log_line(szBuff);
      log_line("");
   }
   log_line("=================================================================");
...
void _hardware_find_usb_radio_interfaces_info()
{
   ...
   log_line("[HardwareRadio]: Finding USB radio interfaces...");
   char szBuff[4096];
   szBuff[0] = 0;
   ...
   while (iPos < iLen )
   {
      ...
      for( int i=iPos; i<iEndLine; i++ )
      {
         ...
         else
         {
            ...
            log_line("[HardwareRadio] Found USB radio device: bus %d, device %d: prod id: %s, name: %s",
            ...
            if ( s_iFoundUSBRadioInterfaces < MAX_USB_DEVICES_INFO-1 )
               ...
               log_softerror_and_alarm("[HardwareRadio] Not enough space in USB radio interfaces info to store one more entry. It has %d entries already.", s_iFoundUSBRadioInterfaces);
            break;
         }
      }
   ...
   log_line("[HardwareRadio]: Done finding USB radio interfaces. Found %d radio interfaces.", s_iFoundUSBRadioInterfaces);
}
...
int hardware_enumerate_radio_interfaces()
{
   ...
   if ( access( FILE_CURRENT_RADIO_HW_CONFIG, R_OK ) != -1 )
   {
      log_line("HW Enumerate: has radio HW info file. Using it.");
      ...
      log_line("HW Enumerate: no exisintg radio HW info file. Enumerating HW from scratch.");
   ...
   if ( NULL == fp )
   {
      log_error_and_alarm("Failed to enumerate radios");
      log_line("=================================================================");
      return 0;
   }
   while (fgets(szBuff, 255, fp) != NULL)
   {
      ...
      sRadioInfo[s_iHwRadiosCount].currentFrequency = 0;
   ...
   {
      log_error_and_alarm("No network cards found!");
      log_line("=================================================================");
      return 0;
   }
   ...
   log_line("=================================================================");
   log_line("Total: %d radios found:", s_iHwRadiosCount);
   hardware_log_radio_info();
...
int hardware_radioindex_supports_frequency(int iRadioIndex, int freq)
{
   if ( ! s_HardwareRadiosEnumeratedOnce )
      hardware_enumerate_radio_interfaces();
   if ( iRadioIndex < 0 || iRadioIndex >= s_iHwRadiosCount )
...
int hardware_radio_supports_frequency(radio_hw_info_t* pRadioInfo, int freq)
{
   if ( -1 == freq || 0 == freq )
      return 0;
   if ( NULL == pRadioInfo )
