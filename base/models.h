...
typedef struct
{
   ...
   int h264profile; // 0 = baseline, 1 = main, 2 = high, 3 = extended
...
typedef struct
{
   ...
   int  interface_current_frequency[MAX_RADIO_INTERFACES]; // current frequency for this card
...
{
   int links_count;
   int link_frequency[MAX_RADIO_INTERFACES];
...
class Model
{
   public:
      ...
      void populateDefaultRadioInterfacesInfo();
      void populateDefaultRadioLinksInfo();
      ...
      int getLinkDataRate(int nLinkId);
      ...
      void populateVehicleTelemetryData(t_packet_header_ruby_telemetry_extended* pdpvt);
      void populateFromVehicleTelemetryData(t_packet_header_ruby_telemetry_extended* pdpvt);
