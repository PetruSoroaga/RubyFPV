...
shared_mem_controller_adaptive_video_info* shared_mem_controller_adaptive_video_info_open_for_read()
{
   void *retVal = open_shared_mem_for_read(SHARED_MEM_CONTROLLER_ADAPTIVE_VIDEO_INFO, sizeof(shared_mem_controller_adaptive_video_info));
   return (shared_mem_controller_adaptive_video_info*)retVal;
}

shared_mem_controller_adaptive_video_info* shared_mem_controller_adaptive_video_info_open_for_write()
{
   void *retVal = open_shared_mem_for_write(SHARED_MEM_CONTROLLER_ADAPTIVE_VIDEO_INFO, sizeof(shared_mem_controller_adaptive_video_info));
   return (shared_mem_controller_adaptive_video_info*)retVal;
}

void shared_mem_controller_adaptive_video_info_close(shared_mem_controller_adaptive_video_info* pAddress)
{
   if ( NULL != pAddress )
      munmap(pAddress, sizeof(shared_mem_controller_adaptive_video_info));
   //shm_unlink(szName);
}
...
t_packet_header_ruby_telemetry_extended* shared_mem_ruby_telemetry_open_for_write(int readOnly)
{
   void *retVal = open_shared_mem(SHARED_MEM_TELEMETRY_RUBY_NAME, sizeof(t_packet_header_ruby_telemetry_extended), 0);
   return (t_packet_header_ruby_telemetry_extended*)retVal;
}  

t_packet_header_ruby_telemetry_extended* shared_mem_ruby_telemetry_open_for_read(int readOnly)
{
   void *retVal = open_shared_mem(SHARED_MEM_TELEMETRY_RUBY_NAME, sizeof(t_packet_header_ruby_telemetry_extended), 1);
   return (t_packet_header_ruby_telemetry_extended*)retVal;
}  

void shared_mem_ruby_telemetry_close(t_packet_header_ruby_telemetry_extended* pAddress)
{
   if ( NULL != pAddress )
      munmap(pAddress, sizeof(t_packet_header_ruby_telemetry_extended));
}

t_packet_header_ruby_telemetry_extended_extra_info_retransmissions* shared_mem_ruby_telemetry_extra_retr_open_for_write(int readOnly)
{
   void *retVal = open_shared_mem(SHARED_MEM_RUBY_TELEMETRY_EXTENDED_RETRANSMISSIONS, sizeof(t_packet_header_ruby_telemetry_extended_extra_info_retransmissions), 0);
   return (t_packet_header_ruby_telemetry_extended_extra_info_retransmissions*)retVal;
}  

t_packet_header_ruby_telemetry_extended_extra_info_retransmissions* shared_mem_ruby_telemetry_extra_retr_open_for_read(int readOnly)
{
   void *retVal = open_shared_mem(SHARED_MEM_RUBY_TELEMETRY_EXTENDED_RETRANSMISSIONS, sizeof(t_packet_header_ruby_telemetry_extended_extra_info_retransmissions), 1);
   return (t_packet_header_ruby_telemetry_extended_extra_info_retransmissions*)retVal;
}  

void shared_mem_ruby_telemetry_extra_retr_close(t_packet_header_ruby_telemetry_extended_extra_info_retransmissions* pAddress)
{
   if ( NULL != pAddress )
      munmap(pAddress, sizeof(t_packet_header_ruby_telemetry_extended_extra_info_retransmissions));
}

t_packet_header_fc_telemetry* shared_mem_fc_telemetry_open(int readOnly)
{
   void *retVal = open_shared_mem(SHARED_MEM_TELEMETRY_FC_NAME, sizeof(t_packet_header_fc_telemetry), readOnly);
   t_packet_header_fc_telemetry *tretval = (t_packet_header_fc_telemetry*)retVal;
   return tretval;
}  

void shared_mem_fc_telemetry_close(t_packet_header_fc_telemetry* pAddress)
{
   if ( NULL != pAddress )
      munmap(pAddress, sizeof(t_packet_header_fc_telemetry));
   //shm_unlink(SHARED_MEM_TELEMETRY_FC_NAME);
}

t_packet_header_fc_extra* shared_mem_fc_extra_telemetry_open(int readOnly)
{
   void *retVal = open_shared_mem(SHARED_MEM_TELEMETRY_FC_EXTRA_NAME, sizeof(t_packet_header_fc_extra), readOnly);
   t_packet_header_fc_extra *tretval = (t_packet_header_fc_extra*)retVal;
   return tretval;
}  

void shared_mem_fc_extra_telemetry_close(t_packet_header_fc_extra* pAddress)
{
   if ( NULL != pAddress )
      munmap(pAddress, sizeof(t_packet_header_fc_extra));
   //shm_unlink(SHARED_MEM_TELEMETRY_FC_MESSAGE_NAME);
}

t_packet_header_fc_rc_channels* shared_mem_fc_rc_channels_open_for_read()
{
   void *retVal =  open_shared_mem(SHARED_MEM_TELEMETRY_FC_RC_CHANNELS, sizeof(t_packet_header_fc_rc_channels), 1);
   return (t_packet_header_fc_rc_channels*)retVal;
}

t_packet_header_fc_rc_channels* shared_mem_fc_rc_channels_open_for_write()
{
   void *retVal =  open_shared_mem(SHARED_MEM_TELEMETRY_FC_RC_CHANNELS, sizeof(t_packet_header_fc_rc_channels), 0);
   return (t_packet_header_fc_rc_channels*)retVal;
}

void shared_mem_fc_rc_channels_close(t_packet_header_fc_rc_channels* pAddress)
{
   if ( NULL != pAddress)
      munmap(pAddress, sizeof(t_packet_header_fc_rc_channels));
}
