...
void _controller_interfaces_add_card(const char* szMAC)
{
   ...
   s_CIS.listRadioInterfaces[s_CIS.radioInterfacesCount].capabilities_flags = RADIO_HW_CAPABILITY_FLAG_CAN_USE_FOR_VIDEO | RADIO_HW_CAPABILITY_FLAG_CAN_USE_FOR_DATA | RADIO_HW_CAPABILITY_FLAG_CAN_USE_FOR_RELAY | RADIO_HW_CAPABILITY_FLAG_CAN_RX | RADIO_HW_CAPABILITY_FLAG_CAN_TX;
...
int load_ControllerInterfacesSettings()
{
   ...
   for( int i=0; i<s_CIS.listMACTXPreferredCount; i++ )
   {
      if ( 1 != fscanf(fd, "%s", s_CIS.listMACTXPreferred[i]) )
      {
         failed = 7;
...
int controllerIsCardTXPreferred(const char* szMAC)
{
   if ( NULL == szMAC || 0 == szMAC[0] )
      return 0;
   for( int i=0; i<s_CIS.listMACTXPreferredCount; i++ )
      if ( 0 == strcmp(szMAC, s_CIS.listMACTXPreferred[i]) )
         return 1;
   return 0;
}
...
void controllerSetCardTXPreferred(const char* szMAC)
{
   if ( NULL == szMAC || 0 == szMAC[0] )
      return;
   if ( 1 == controllerIsCardTXPreferred(szMAC) )
      return;

   if ( s_CIS.listMACTXPreferredCount >= CONTROLLER_INTERFACES_MAX_LIST_SIZE )
      return;
   log_line("Adding radio interface %s to preffered TX list", szMAC);
   strcpy(s_CIS.listMACTXPreferred[s_CIS.listMACTXPreferredCount], szMAC);
   s_CIS.listMACTXPreferredCount++;
   save_ControllerInterfacesSettings();
}
...
void controllerRemoveCardTXPreferred(const char* szMAC)
{
   if ( NULL == szMAC || 0 == szMAC[0] )
      return;
   if ( 0 == controllerIsCardTXPreferred(szMAC) )
      return;
   for( int i=0; i<s_CIS.listMACTXPreferredCount; i++ )
      if ( 0 == strcmp(szMAC, s_CIS.listMACTXPreferred[i]) )
      {
         for( int j=i; j<s_CIS.listMACTXPreferredCount-1; j++ )
            strcpy(s_CIS.listMACTXPreferred[j], s_CIS.listMACTXPreferred[j+1]);
         s_CIS.listMACTXPreferredCount--;
         break;
      }
...
{
   // Returns the TX preffered index (if any) for this card or -1;
   // Larger numers -> more preffered card

   for( int k=s_CIS.listMACTXPreferredCount-1; k >= 0; k-- )
      if ( 0 == strcmp(szMAC, s_CIS.listMACTXPreferred[k]) )
         return k;
   return -1;
}
...
int _controllerComputeRXTXCardsSingleFrequency(Model* pModel, int* pFrequencies, int* pRXCardIndexes, int& countCardsRX, int* pTXCardIndexes, int& countCardsTX)
{
   ...
   for( int i=0; i<hardware_get_radio_interfaces_count(); i++ )
   {
      ...
      if ( (flags & RADIO_HW_CAPABILITY_FLAG_CAN_USE_FOR_VIDEO) || (flags & RADIO_HW_CAPABILITY_FLAG_CAN_USE_FOR_DATA) || (flags & RADIO_HW_CAPABILITY_FLAG_CAN_USE_FOR_RELAY) )
      {
         *pRXCardIndexes = i;
         pRXCardIndexes++;
         countCardsRX++;
      ...
      if ( (flags & RADIO_HW_CAPABILITY_FLAG_CAN_USE_FOR_DATA) || (flags & RADIO_HW_CAPABILITY_FLAG_CAN_USE_FOR_RELAY) )
      {
         *pTXCardIndexes = i;
         pTXCardIndexes++;
         countCardsTX++;
...
int _controllerComputeRXTXCardsDualFrequency(Model* pModel, int* pFrequencies, int* pRXCardIndexes, int& countCardsRX, int* pTXCardIndexes, int& countCardsTX)
{
   ...
   for( int i=0; i<hardware_get_radio_interfaces_count(); i++ )
   {
      ...
      if ( flags & RADIO_HW_CAPABILITY_FLAG_CAN_RX )
      if ( (flags & RADIO_HW_CAPABILITY_FLAG_CAN_USE_FOR_VIDEO) || (flags & RADIO_HW_CAPABILITY_FLAG_CAN_USE_FOR_VIDEO) || (flags & RADIO_HW_CAPABILITY_FLAG_CAN_USE_FOR_RELAY) )
      {
         *pRXCardIndexes = i;
         pRXCardIndexes++;
         countCardsRX++;
      ...
      if ( flags & RADIO_HW_CAPABILITY_FLAG_CAN_TX )
      if ( (flags & RADIO_HW_CAPABILITY_FLAG_CAN_USE_FOR_VIDEO) || (flags & RADIO_HW_CAPABILITY_FLAG_CAN_USE_FOR_RELAY) )
      {
