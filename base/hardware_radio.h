...
#define RADIO_HW_CAPABILITY_FLAG_CAN_USE_FOR_RELAY  ((u32)(((u32)0x01)<<4))
...
typedef struct
{
   ...
   char szProductId[12];
   char szUSBPort[6];  // [A-X], [A-X][1-9], ...
   int typeAndDriver; // first byte: radio type, second byte = driver type
   int openedForRead;
   int openedForWrite;
   monitor_wlan_interface_t monitor_interface_read;
   monitor_wlan_interface_t monitor_interface_write;
} radio_hw_info_t;
