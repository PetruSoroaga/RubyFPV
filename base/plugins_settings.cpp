...
int save_PluginsSettings()
{
   ...
   for( int iPlugin=0; iPlugin<s_PluginsSettings.nPluginsCount; iPlugin++ )
   {
      ...
      for( int iModel=0; iModel<s_PluginsSettings.pPlugins[iPlugin]->nModels; iModel++ )
      {
         ...
         for( int k=0; k<MODEL_MAX_OSD_PROFILES; k++ )
            fprintf(fd, "%d ", s_PluginsSettings.pPlugins[iPlugin]->nOSDEnabled[iModel][k]);
         fprintf(fd, "\n");
   ...
   log_line("Saved plugins settings to file: %s", FILE_OSD_PLUGINS_SETTINGS);
   return 1;
}
...
int load_PluginsSettings()
{
   ...
   for( int iPlugin=0; iPlugin<s_PluginsSettings.nPluginsCount; iPlugin++ )
   {
      ...
      for( int iModel=0; iModel<s_PluginsSettings.pPlugins[iPlugin]->nModels; iModel++ )
      {
         ...
         for( int k=0; k<MODEL_MAX_OSD_PROFILES; k++ )
            if ( (!failed) && (1 != fscanf(fd, "%d", &s_PluginsSettings.pPlugins[iPlugin]->nOSDEnabled[iModel][k]) ) )
               { failed = 1; log_softerror_and_alarm("Load plugins settings: Invalid plugin OSD setting [%d/%d/%d].", iPlugin, iModel, k); }
...
int getPluginModelSettingsIndex(SinglePluginSettings* pPlugin, Model* pModel)
{
   ...
   if ( pPlugin->nModels >= MAX_MODELS )
   {
      ...
      for( int i=0; i<MAX_MODELS-1; i++ )
      {
         ...
         for( int k=0; k<MODEL_MAX_OSD_PROFILES; k++ )
         {
            ...
            pPlugin->nOSDEnabled[i][k] = pPlugin->nOSDEnabled[i+1][k];
            for( int j=0; j<MAX_PLUGIN_SETTINGS; j++ )
               pPlugin->nSettings[i][k][k] = pPlugin->nSettings[i+1][k][j];
         }
      }
      pPlugin->nModels--;   
   }
   ...
   if ( pPlugin->nModels > 0 )
   {
      ...
      for( int k=0; k<MODEL_MAX_OSD_PROFILES; k++ )
      {
         ...
         pPlugin->nOSDEnabled[pPlugin->nModels][k] = pPlugin->nOSDEnabled[pPlugin->nModels-1][k];
         for( int j=0; j<MAX_PLUGIN_SETTINGS; j++ )
            pPlugin->nSettings[pPlugin->nModels][k][k] = pPlugin->nSettings[pPlugin->nModels-1][k][j];
      }
   }
...
void deletePluginModelSettings(u32 uModelId)
{
   ...
   for( int i=0; i<s_PluginsSettings.nPluginsCount; i++ )
   {
      for( int iModel=0; iModel<s_PluginsSettings.pPlugins[i]->nModels; iModel++ )
         if ( s_PluginsSettings.pPlugins[i]->uModelsIds[iModel] == uModelId )
         {
            for( int k=iModel; k<s_PluginsSettings.pPlugins[i]->nModels-1; k++ )
            {
               ...
               s_PluginsSettings.pPlugins[i]->nOSDEnabled[k][j] = s_PluginsSettings.pPlugins[i]->nOSDEnabled[k+1][j];
