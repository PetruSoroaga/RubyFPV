...
t_packet_header_ruby_telemetry_extended_extra_info_retransmissions* s_pPHRTE_Retransmissions;
t_packet_header_fc_telemetry* s_pPHFCT = NULL;
t_packet_header_fc_extra* s_pPHFCE = NULL;
t_packet_header_fc_rc_channels* s_pPHFCRCChannels = NULL;
...
void _process_data_ruby_telemetry_extended(u8* pBuffer, int length)
{
   t_packet_header* pPH = (t_packet_header*)pBuffer;
   t_packet_header_ruby_telemetry_extended* pPHRTE = (t_packet_header_ruby_telemetry_extended*)(pBuffer+sizeof(t_packet_header));

   //log_line("recv ruby telemetry, vid: %u", pPHRTE->vehicle_id);

   if ( NULL != s_pPHRTE )
      memcpy(s_pPHRTE, pBuffer+sizeof(t_packet_header), sizeof(t_packet_header_ruby_telemetry_extended) );
   ...
   if ( NULL != s_pPHRTE_Retransmissions)
   {
      memcpy(s_pPHRTE_Retransmissions, pBuffer + sizeof(t_packet_header) + sizeof(t_packet_header_ruby_telemetry_extended), sizeof(t_packet_header_ruby_telemetry_extended_extra_info_retransmissions));
   }
...
void _process_data_fc_telemetry(u8* pBuffer, int length)
{
   if ( NULL != s_pPHFCT )
      memcpy(s_pPHFCT, pBuffer+sizeof(t_packet_header), sizeof(t_packet_header_fc_telemetry) );

   if ( NULL != g_pProcessStats )
      g_pProcessStats->timeLastReceivedPacket = g_TimeNow;
}

void _process_data_fc_telemetry_extended(u8* pBuffer, int length)
{
   if ( NULL != s_pPHFCT )
      memcpy(s_pPHFCT, pBuffer+sizeof(t_packet_header), sizeof(t_packet_header_fc_telemetry) );
   if ( NULL != s_pPHFCE )
      memcpy(s_pPHFCE, pBuffer+sizeof(t_packet_header) + sizeof(t_packet_header_fc_telemetry), sizeof(t_packet_header_fc_extra) );
...
void _process_data_fc_rc_channels(u8* pBuffer, int length)
{
   if ( NULL != s_pPHFCRCChannels )
      memcpy(s_pPHFCRCChannels, pBuffer+sizeof(t_packet_header), sizeof(t_packet_header_fc_rc_channels) );
...
void try_read_messages_from_router()
{
   ...
   while ( (maxMessagesToRead > 0) && NULL != ruby_ipc_try_read_message(s_fIPCFromRouter, timeoutReadPipeMicroseconds, s_PipeBufferTelemetryDownlink, &s_PipeBufferTelemetryDownlinkPos, s_BufferTelemetryDownlink) )
   {
      ...
      else if ( pPH->packet_type == PACKET_TYPE_RUBY_TELEMETRY_EXTENDED )
         _process_data_ruby_telemetry_extended(s_BufferTelemetryDownlink, pPH->total_length);
      else if ( pPH->packet_type == PACKET_TYPE_FC_TELEMETRY )
         _process_data_fc_telemetry(s_BufferTelemetryDownlink, pPH->total_length);
      else if ( pPH->packet_type == PACKET_TYPE_FC_TELEMETRY_EXTENDED )
         _process_data_fc_telemetry_extended(s_BufferTelemetryDownlink, pPH->total_length);
      else if ( pPH->packet_type == PACKET_TYPE_RC_TELEMETRY )
         _process_data_rc_telemetry(s_BufferTelemetryDownlink, pPH->total_length);
      else if ( pPH->packet_type == PACKET_TYPE_FC_RC_CHANNELS )
         _process_data_fc_rc_channels(s_BufferTelemetryDownlink, pPH->total_length);
...
void init_serial_ports()
{
   ...
   if ( portIndex == -1 || portIndex >= hardware_get_serial_ports_count() )
   {
...
void open_shared_mem_objects()
{
   ...
   s_pPHRTE_Retransmissions = shared_mem_ruby_telemetry_extra_retr_open_for_write();
   s_pPHFCT = shared_mem_fc_telemetry_open(false);
   s_pPHFCE = shared_mem_fc_extra_telemetry_open(false);
   
   if ( NULL == s_pPHRTE )
      log_softerror_and_alarm("Failed to open ruby telemetry shared memory for write.");
   else
      log_line("Opened ruby telemetry shared memory for write: success.");

   if ( NULL == s_pPHRTE_Retransmissions )
      log_softerror_and_alarm("Failed to open ruby telemetry extended info retransmissions shared memory for write.");
   else
      log_line("Opened ruby telemetry extended info retransmissions shared memory for write: success.");

   if ( NULL == s_pPHFCT )
      log_softerror_and_alarm("Failed to open fc telemetry shared memory for write.");
   else
      log_line("Opened fc telemetry shared memory for write: success.");

   if ( NULL == s_pPHFCE )
      log_softerror_and_alarm("Failed to open fc extra telemetry shared memory for write.");
   else
      log_line("Opened fc extra telemetry shared memory for write: success.");
   

   s_pPHFCRCChannels = shared_mem_fc_rc_channels_open_for_write();
   if ( NULL == s_pPHFCRCChannels )
      log_softerror_and_alarm("Failed to open fc rc channels shared memory for write.");
   else
      log_line("Opened fc rc channels shared memory for write: success.");
   ...
   if ( NULL != s_pPHFCT )
     s_pPHFCT->flags |= FC_TELE_FLAGS_NO_FC_TELEMETRY;

   if ( NULL != s_pPHFCE )
   {
      s_pPHFCE->chunk_index = 0;
      s_pPHFCE->text[0] = 0;
   }
...
int main(int argc, char *argv[])
{
   ...
   shared_mem_ruby_telemetry_extra_retr_close(s_pPHRTE_Retransmissions);
   shared_mem_fc_telemetry_close(s_pPHFCT);
   shared_mem_fc_extra_telemetry_close(s_pPHFCE);
   shared_mem_fc_rc_channels_close(s_pPHFCRCChannels);
