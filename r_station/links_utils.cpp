...
bool links_set_cards_frequencies_for_search( int iSearchFreq )
{
   log_line("Links: Set all cards frequencies for search mode to %d Mhz", iSearchFreq);
   for( int i=0; i<hardware_get_radio_interfaces_count(); i++ )
   {
      ...
      {
         launch_set_frequency(NULL, i, iSearchFreq); 
         g_Local_RadioStats.radio_interfaces[i].currentFrequency = iSearchFreq;
         radio_stats_set_card_current_frequency(&g_Local_RadioStats, i, iSearchFreq);
      }
   }
   if ( NULL != g_pSM_RadioStats )
      memcpy((u8*)g_pSM_RadioStats, (u8*)&g_Local_RadioStats, sizeof(shared_mem_radio_stats));
   log_line("Links: Set all cards frequencies for search mode to %d Mhz. Completed.", iSearchFreq);
   return true;
}
