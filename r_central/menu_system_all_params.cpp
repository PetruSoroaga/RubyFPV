...
float MenuSystemAllParams::renderRadioInfo(float xPos, float yPos, float width, float height, float fScale)
{
   ...
   for( int i=0; i<hardware_get_radio_interfaces_count(); i++ )
   {
      ...
      if ( pNICInfo->lastFrequencySetFailed )
         ...
         sprintf(szBuff, "   Now at: %d Mhz, bands:", pNICInfo->currentFrequency);
      ...
      for( int i=0; i<g_pCurrentModel->radioInterfacesParams.interfaces_count; i++ )
         {
            ...
            g_pRenderEngine->drawText(xPos, yPos, height_text, g_idFontMenu, szBuff);
            yPos += 1.4*height_text;
            sprintf(szBuff, "   Now at %d Mhz, bands: ", g_pCurrentModel->radioInterfacesParams.interface_current_frequency[i]);
...
float MenuSystemAllParams::renderDataRates(float xPos, float yPos, float width, float fScale)
{
   ...
   for( int i=0; i<hardware_get_radio_interfaces_count(); i++ )
   {
      ...
      {
         sprintf(szTemp, "MCS-%d (%d Mbps)", -dataRate-1, getMCSRate(-dataRate-1));
         strcat(szBuff, szTemp);
      }
      else
      {
         sprintf(szTemp, "%d Mbps", dataRate);
         strcat(szBuff, szTemp);
      }
   ...
   for( int i=0; i<g_pCurrentModel->radioInterfacesParams.interfaces_count; i++ )
   {
      ...
      {
         sprintf(szTemp, "MCS-%d (%d Mbps), STBC: %s", -g_pCurrentModel->radioInterfacesParams.interface_datarates[i][0]-1, getMCSRate(-g_pCurrentModel->radioInterfacesParams.interface_datarates[i][0]-1), (g_pCurrentModel->radioInterfacesParams.interface_current_radio_flags[i] & RADIO_FLAGS_STBC)?"Yes":"No");
         strcat(szBuff, szTemp);
      }
      else
      {
         sprintf(szTemp, "%d Mbps", g_pCurrentModel->radioInterfacesParams.interface_datarates[i][0]);
         strcat(szBuff, szTemp);
      }
   }
   yPos += g_pRenderEngine->drawMessageLines(xPos, yPos, szBuff, height_text, MENU_TEXTLINE_SPACING, width, g_idFontMenu);
