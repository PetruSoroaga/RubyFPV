...
float osd_show_radio_link_new(float xPos, float yPos, int iRadioLink, bool bHorizontal)
{
   ...
   if ( (! bShowInterfaces) && (! bShowInterfacesExtended) )
       dySignalBars = height_text_small*0.4;
   ...
   if ( ! (g_pCurrentModel->radioLinksParams.link_capabilities_flags[iRadioLink] & RADIO_HW_CAPABILITY_FLAG_DISABLED) )
   {
      if ( bShowSummary )
      {
         ...
         for( int i=0; i<g_pCurrentModel->radioInterfacesParams.interfaces_count; i++ )
         {
            if ( g_pCurrentModel->radioInterfacesParams.interface_link_id[i] != iRadioLink )
               continue;
            if ( g_pPHRTE->uplink_link_quality[i] > nRxQuality )
               nRxQuality = g_pPHRTE->uplink_link_quality[i];
            ...
            else if ( g_pPHRTE->uplink_datarate[i]/2 > iDataRate )
               iDataRate = g_pPHRTE->uplink_datarate[i]/2;
         }
         float fSize = _osd_show_radio_bars_info(xPos, yPos+dySignalBars, nRxQuality, iDataRate, bShowBars, bShowPercentages, true, bHorizontal, false);
         if ( bHorizontal )
         {
            xPos -= fSize;
         ...
         _osd_show_radio_bars_info(xPos, yPos+dySignalBars, nRxQuality, iDataRate, bShowBars, bShowPercentages, true, bHorizontal, true);
         if ( ! bHorizontal )
         {
             yPos += fSize;
      ...
      else // Not summary
      {
         ...
         for( int i=0; i<g_pCurrentModel->radioInterfacesParams.interfaces_count; i++ )
         {
            if ( g_pCurrentModel->radioInterfacesParams.interface_link_id[i] != iRadioLink )
               continue;
            int nRxQuality = g_pPHRTE->uplink_link_quality[i];
            int iDataRate = g_pPHRTE->uplink_datarate[i]/2;
            if ( g_pCurrentModel->radioLinksParams.link_datarates[iRadioLink][0] < 0 )
                iDataRate = g_pCurrentModel->radioLinksParams.link_datarates[iRadioLink][0];
            if ( iCountShown > 0 )
            {
            ...
            int dbm = g_pPHRTE->uplink_rssi_dbm[i]-200;
            if ( dbm < -110 )
            {
            ...
            float fWidthLine1 = g_pRenderEngine->textWidth(0, g_idFontOSDSmall, szLine1);
            if ( bHorizontal )
               g_pRenderEngine->drawText(xPos + (fSize - fWidthLine1)*0.5, yPos + osd_get_link_bars_height(1.0), 0.0, g_idFontOSDSmall, szLine1);
            else
            {
               g_pRenderEngine->drawText(xStart + (osd_getVerticalBarWidth() - fWidthLine1)*0.5, yPos, 0.0, g_idFontOSDSmall, szLine1);
      ...
      else // Not summary
      {
         ...
         for( int i=0; i<g_pSM_RadioStats->countRadioInterfaces; i++ )
         {
            ...
            if ( bShowInterfacesExtended )
            {
               ...
               if ( bHorizontal )
                  g_pRenderEngine->drawText(xPos + (fSize - fWidthLine1)*0.5, yPos + osd_get_link_bars_height(1.0) + height_text_small, g_idFontOSDExtraSmall, szLine1);
               else
               {
                  g_pRenderEngine->drawText(xStart + (osd_getVerticalBarWidth() - fWidthLine1)*0.5, yPos, g_idFontOSDExtraSmall, szLine1);
                  yPos += height_text_small;
                  fTotalHeightLink += height_text_small;             
               }
            }
