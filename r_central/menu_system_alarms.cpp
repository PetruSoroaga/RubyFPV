...
:Menu(MENU_ID_SYSTEM_ALARMS, "Configure System Alarms", NULL)
{
   ...
   m_pItemsSelect[3] = new MenuItemSelect("Vehicle CPU Overload", "Turn this alarm on or off.");
   ...
   m_pItemsSelect[4] = new MenuItemSelect("Controller CPU Overload", "Turn this alarm on or off.");
...
void MenuSystemAlarms::onSelectItem()
{
   ...
   if ( m_IndexAlarmVehicleCPUOverload == m_SelectedIndex )
   {
      if ( 0 == m_pItemsSelect[3]->getSelectedIndex() )
         pP->uEnabledAlarms &= ~(ALARM_ID_VEHICLE_CPU_LOOP_OVERLOAD);
      else
         pP->uEnabledAlarms |= (ALARM_ID_VEHICLE_CPU_LOOP_OVERLOAD);
   }
