...
void MenuVehicleInstrumentsGeneral::valuesToUI()
{
   ...
   m_pItemsSelect[3]->setSelection((g_pCurrentModel->osd_params.osd_flags2[m_nOSDIndex] & OSD_FLAG_EXT_RELATIVE_ALTITUDE)?1:0);
   m_pItemsSelect[6]->setSelection((g_pCurrentModel->osd_params.osd_flags2[m_nOSDIndex] & OSD_FLAG_EXT_SHOW_LOCAL_VERTICAL_SPEED)?1:0);
...
void MenuVehicleInstrumentsGeneral::onSelectItem()
{
   ...
   if ( m_IndexAltitudeType == m_SelectedIndex )
   {
      if ( 0 == m_pItemsSelect[3]->getSelectedIndex() )
      {
         ...
         for( int i=0; i<MODEL_MAX_OSD_PROFILES; i++ )
            params.osd_flags2[i] &= ~OSD_FLAG_EXT_RELATIVE_ALTITUDE;
      }
      else
      {
   ...
   if ( m_IndexLocalVertSpeed == m_SelectedIndex )
   {
      if ( 0 == m_pItemsSelect[6]->getSelectedIndex() )
         for( int i=0; i<MODEL_MAX_OSD_PROFILES; i++ )
            params.osd_flags2[i] &= ~OSD_FLAG_EXT_SHOW_LOCAL_VERTICAL_SPEED;
      else
         for( int i=0; i<MODEL_MAX_OSD_PROFILES; i++ )
            params.osd_flags2[i] |= OSD_FLAG_EXT_SHOW_LOCAL_VERTICAL_SPEED;
      sendToVehicle = true;
