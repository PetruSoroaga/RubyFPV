...
void _osd_plugins_populate_public_telemetry_info()
{
   if ( NULL == g_pPHRTE )
      return;
   g_VehicleTelemetryInfo.pi_temperature = g_pPHRTE->temperature;
   g_VehicleTelemetryInfo.cpu_load = g_pPHRTE->cpu_load;
   g_VehicleTelemetryInfo.cpu_mhz = g_pPHRTE->cpu_mhz;
   g_VehicleTelemetryInfo.throttled = g_pPHRTE->throttled;
   g_VehicleTelemetryInfo.rssi_dbm = g_pPHRTE->uplink_rssi_dbm[0];
   g_VehicleTelemetryInfo.rssi_quality = g_pPHRTE->uplink_link_quality[0];
   strncpy(&g_VehicleTelemetryInfo.vehicle_name[0], (char*)&(g_pPHRTE->vehicle_name[0]), MAX_VEHICLE_NAME_LENGTH);
   g_VehicleTelemetryInfo.vehicle_name[MAX_VEHICLE_NAME_LENGTH-1] = 0;
   g_VehicleTelemetryInfo.vehicle_type = g_pPHRTE->vehicle_type;

   if ( NULL == g_pdpfct )
      return;

   g_VehicleTelemetryInfo.flags = g_pdpfct->flags;
   g_VehicleTelemetryInfo.flight_mode = g_pdpfct->flight_mode;
   g_VehicleTelemetryInfo.arm_time = g_pdpfct->arm_time;
   g_VehicleTelemetryInfo.throttle = g_pdpfct->throttle;
   g_VehicleTelemetryInfo.voltage = g_pdpfct->voltage;
   g_VehicleTelemetryInfo.current = g_pdpfct->current;
   g_VehicleTelemetryInfo.voltage2 = g_pdpfct->voltage2;
   g_VehicleTelemetryInfo.current2 = g_pdpfct->current2;
   g_VehicleTelemetryInfo.mah = g_pdpfct->mah;
   g_VehicleTelemetryInfo.altitude = g_pdpfct->altitude;
   g_VehicleTelemetryInfo.altitude_abs = g_pdpfct->altitude_abs;
   g_VehicleTelemetryInfo.distance = g_pdpfct->distance;
   g_VehicleTelemetryInfo.total_distance = g_pdpfct->total_distance;

   g_VehicleTelemetryInfo.vspeed = g_pdpfct->vspeed;
   g_VehicleTelemetryInfo.aspeed = g_pdpfct->aspeed;
   g_VehicleTelemetryInfo.hspeed = g_pdpfct->hspeed;
   //g_VehicleTelemetryInfo.roll = g_pdpfct->roll;
   //g_VehicleTelemetryInfo.pitch = g_pdpfct->pitch;
   g_VehicleTelemetryInfo.roll = g_pdpfct->roll/100.0-180.0;
   g_VehicleTelemetryInfo.pitch = g_pdpfct->pitch/100.0-180.0;
   ...
   g_VehicleTelemetryInfo.satelites = g_pdpfct->satelites;
   g_VehicleTelemetryInfo.gps_fix_type = g_pdpfct->gps_fix_type;
   g_VehicleTelemetryInfo.hdop = g_pdpfct->hdop;
   g_VehicleTelemetryInfo.heading = g_pdpfct->heading;
   g_VehicleTelemetryInfo.latitude = g_pdpfct->latitude;
   g_VehicleTelemetryInfo.longitude = g_pdpfct->longitude;

   g_VehicleTelemetryInfo.rc_rssi = g_pdpfct->rc_rssi;
   memcpy(&(g_VehicleTelemetryInfo.extra_info[0]), &(g_pdpfct->extra_info[0]), sizeof(u8)*12);
...
void osd_plugins_render()
{
   ...
   for( int i=0; i<g_iPluginsOSDCount; i++ )
   {
      ...
      if ( !(g_pCurrentModel->osd_params.instruments_flags[s_nCurrentOSDFlagsIndex] & (INSTRUMENTS_FLAG_SHOW_FIRST_OSD_PLUGIN<<i)) )
         continue;
      ...
      telemetry_info2.uIsSpectatorMode = 1;
      if ( NULL != g_pCurrentModel )
      {
      ...
      plugin_settings.pExtraInfo = NULL;
