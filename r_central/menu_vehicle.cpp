...
void MenuVehicle::onShow()
{
   ...
   index = addMenuItem(new MenuItem("Camera","Change camera parameters: brightness, contrast, sharpness and so on."));
   if ( NULL != g_pCurrentModel && (!g_pCurrentModel->hasCamera()) )
      m_pMenuItems[index]->setEnabled(false);
   if ( NULL != g_pCurrentModel && g_pCurrentModel->is_spectator )
      m_pMenuItems[index]->setEnabled(false);

   index = addMenuItem(new MenuItem("Video","Change video resolution, fps so on."));
   if ( NULL != g_pCurrentModel && (!g_pCurrentModel->hasCamera()) )
      m_pMenuItems[index]->setEnabled(false);
   if ( NULL != g_pCurrentModel && g_pCurrentModel->is_spectator )
      m_pMenuItems[index]->setEnabled(false);

   index = addMenuItem(new MenuItem("Audio","Change the audio settings"));
   if ( NULL != g_pCurrentModel && g_pCurrentModel->is_spectator )
      m_pMenuItems[index]->setEnabled(false);
   ...
   index = addMenuItem(new MenuItem("OSD / Instruments","Change OSD type, layout and settings."));
    
   index = addMenuItem(new MenuItem("Remote Control","Change your remote control type, channels, protocols and so on.")); 
...
void MenuVehicle::onSelectItem()
{
   ...
   if ( NULL != g_pCurrentModel && g_pCurrentModel->is_spectator )
   {
      if ( (7 != m_SelectedIndex) && ( 8 != m_SelectedIndex) )
      {
   ...
   //if ( 9 == m_SelectedIndex )
   //{
   //   add_menu_to_stack(new MenuVehicleRadioLinks());
   //   return;
   //}
