...
void MenuVehicleGeneral::populate()
{
   ...
   for( int n=0; n<g_pCurrentModel->radioLinksParams.links_count; n++ )
   {
      ...
      m_SupportedChannelsCount[n] = getSupportedChannels( controllerBands & g_pCurrentModel->radioInterfacesParams.interface_supported_bands[iRadioInterfaceId], 1, &(m_SupportedChannels[n][0]), 100);
      ...
      {
         sprintf(szBuff, "Radio Link %d Frequency", n+1 );
         sprintf(szTooltip, "Sets the radio link %d frequency used by the vehicle and the controller to communicate with each other.", n+1);
      }

      m_pItemsSelect[20+n] = new MenuItemSelect(szBuff, szTooltip);

      for( int ch=0; ch<m_SupportedChannelsCount[n]; ch++ )
      {
         if ( -1 == m_SupportedChannels[n][ch] )
         {
            m_pItemsSelect[20+n]->addSeparator();
            continue;
         }
         sprintf(szBuff,"%d Mhz", m_SupportedChannels[n][ch]);
         m_pItemsSelect[20+n]->addSelection(szBuff);
      }
      m_SupportedChannelsCount[n] = getSupportedChannels( controllerBands & g_pCurrentModel->radioInterfacesParams.interface_supported_bands[iRadioInterfaceId], 0, &(m_SupportedChannels[n][0]), 100);
      ...
      m_pItemsSelect[20+n]->setSelection(selectedIndex);

      m_pItemsSelect[20+n]->setIsEditable();
      if ( g_pCurrentModel->radioLinksParams.link_capabilities_flags[n] & RADIO_HW_CAPABILITY_FLAG_DISABLED )
         m_pItemsSelect[20+n]->setEnabled(false);

      m_IndexFreq[n] = addMenuItem(m_pItemsSelect[20+n]);
   }
...
void MenuVehicleGeneral::onReturnFromChild(int returnValue)
{
   ...
   {
      u32 param = s_iTempGenNewFrequency;
      param = param | (s_iTempGenNewFrequencyLink<<16);
