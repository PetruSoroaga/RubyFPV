...
void notification_add_frequency_changed(int nLinkId, int nFreq)
{
   char szBuff[64];
   szBuff[0] = 0;
   if ( NULL != g_pCurrentModel && 1 == g_pCurrentModel->radioLinksParams.links_count )
      sprintf(szBuff,"Radio link switched to %d Mhz", nFreq);
   else
      sprintf(szBuff,"Radio link %d switched to %d Mhz", nFreq, nLinkId+1);
