...
void Popup::computeSize()
{
   ...
   if ( 0 != m_szTitle[0] )
   {
      ...
      m_RenderHeight += fHeight + height_text*POPUP_LINE_SPACING;
   }
...
void Popup::Render()
{
   ...
   if ( 0 != m_idIcon )
   {
      ...
      if ( m_RenderHeight-2.0*fPaddingY > 3.0*height_text )
         m_fIconSize = height_text*3.2;
      if ( m_RenderHeight-2.0*fPaddingY >= 4.0*height_text )
   ...
   float fMaxTextWidth = m_RenderWidth - 2.0*fPaddingX + 2.0*g_pRenderEngine->getPixelWidth();
   
   if ( 0 != m_szTitle[0] )
   {
      float fHeightText = g_pRenderEngine->drawMessageLines(x, y, m_szTitle, height_text, POPUP_LINE_SPACING, fMaxTextWidth, m_idFont);
      y += fHeightText + height_text*POPUP_LINE_SPACING;
   }
