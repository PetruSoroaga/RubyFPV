...
void _pairing_open_shared_mem()
{
   ...
   for( int i=0; i<20; i++ )
   {
      if ( NULL != g_pPHRTE_Retransmissions )
         break;
      g_pPHRTE_Retransmissions = shared_mem_ruby_telemetry_extra_retr_open_for_read();
      hardware_sleep_ms(5);
      iAnyNewOpen++;
   }
   if ( NULL == g_pPHRTE_Retransmissions )
   ...
   for( int i=0; i<20; i++ )
   {
      if ( NULL != g_pPHRTE )
         break;
      g_pPHRTE = shared_mem_ruby_telemetry_open_for_read();
      hardware_sleep_ms(5);
      iAnyNewOpen++;
   }
   if ( NULL == g_pPHRTE )
   ...
   if ( s_bDebugOSDShowAll )
   {
      if ( NULL == g_pdpfct )
      {
         g_pdpfct = (t_packet_header_fc_telemetry*)malloc(sizeof(t_packet_header_fc_telemetry));
         memset(g_pdpfct, 0, sizeof(t_packet_header_fc_telemetry));
      }
   }
   else
   {
      for( int i=0; i<20; i++ )
      {
         if ( NULL != g_pdpfct )
            break;
         g_pdpfct = shared_mem_fc_telemetry_open(true);
         hardware_sleep_ms(5);
         iAnyNewOpen++;
      }
   ...
   for( int i=0; i<20; i++ )
   {
      if ( NULL != g_pdpfce )
         break;
      g_pdpfce = shared_mem_fc_extra_telemetry_open(true);
      hardware_sleep_ms(5);
      iAnyNewOpen++;
   }
   if ( NULL == g_pdpfce )
   ...
   for( int i=0; i<20; i++ )
   {
      if ( NULL != g_pPHFCRCChannels )
         break;
      g_pPHFCRCChannels = shared_mem_fc_rc_channels_open_for_read();
      hardware_sleep_ms(5);
      iAnyNewOpen++;
   }
   if ( NULL == g_pPHFCRCChannels )
   ...
   for( int i=0; i<20; i++ )
   {
      if ( NULL != g_pSM_ControllerAdaptiveVideoInfo )
         break;
      g_pSM_ControllerAdaptiveVideoInfo = shared_mem_controller_adaptive_video_info_open_for_read();
      hardware_sleep_ms(5);
      iAnyNewOpen++;
   }
   if ( NULL == g_pSM_ControllerAdaptiveVideoInfo )
...
void _pairing_close_shared_mem()
{
   shared_mem_ruby_telemetry_close(g_pPHRTE);
   g_pPHRTE = NULL;

   shared_mem_ruby_telemetry_extra_retr_close(g_pPHRTE_Retransmissions);
   g_pPHRTE_Retransmissions = NULL;

   if ( s_bDebugOSDShowAll )
      free(g_pdpfct);
   else
      shared_mem_fc_telemetry_close(g_pdpfct);
   g_pdpfct = NULL;

   shared_mem_controller_adaptive_video_info_close(g_pSM_ControllerAdaptiveVideoInfo);
   g_pSM_ControllerAdaptiveVideoInfo = NULL;
   ...
   shared_mem_fc_extra_telemetry_close(g_pdpfce);
   g_pdpfce = NULL;
  
   shared_mem_fc_rc_channels_close(g_pPHFCRCChannels);
   g_pPHFCRCChannels = NULL;
...
bool pairing_is_connected_to_wrong_model()
{
   ...
   if ( 0 != g_pPHRTE->vehicle_id ) // check only if received any ruby telemetry
   if ( g_pPHRTE->vehicle_id != g_pCurrentModel->vehicle_id )
      return true;
...
void pairing_loop()
{
   ...
   if ( ! s_bPairingReceivedAnything )
   {
      for( int i=0; i<g_pSM_RadioStats->countRadioInterfaces; i++ )
      {
         ...
         //if ( NULL != g_pPHRTE && 0 != g_pPHRTE->vehicle_id )
         {
