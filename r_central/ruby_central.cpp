...
void ruby_reload_osd_fonts()
{
   ...
   float hFontExtraSmall = hFont*0.5;
...
void render_all(u32 timeNow, bool bForceBackground, bool bDoInputLoop)
{
   ...
   //log_line("DEBUG central-startrender");
   ...
   if ( g_bIsRouterReady )
   {
      ...
      if ( NULL != g_pCurrentModel && s_nCurrentOSDFlagsIndex >= 0 && s_nCurrentOSDFlagsIndex < MODEL_MAX_OSD_PROFILES )
   ...
   //log_line("DEBUG central-popuprender");

   u32 t = get_current_timestamp_micros();
   popups_render();

   //log_line("DEBUG central-menurender");

   menu_render();

   //log_line("DEBUG central-toppopuprender");
   ...
   //log_line("DEBUG central-endrender");
...
void executeQuickActions()
{
        ...
        (isKeyQA3Pressed() && quickActionRelaySwitch == p->iActionQuickButton3) )
   {
      ...
      if ( ! g_pCurrentModel->relay_params.is_RelayEnabled )
      {
         char szBuff[128];
      ...
      popups_add_topmost(new Popup("Switching relay.", 0.1,0.8, 0.2, 4));
      return;
   }
...
void ruby_load_models()
{
   ...
   if ( NULL != g_pCurrentModel )
   {
      ...
      log_line("Current model radio links: %d, 1st radio link frequency: %d Mhz, 2nd radio link: %d Mhz", g_pCurrentModel->radioLinksParams.links_count, g_pCurrentModel->radioLinksParams.link_frequency[0], g_pCurrentModel->radioLinksParams.link_frequency[1]);
   }
}
   ...
   if ( s_StartSequence == START_SEQ_NICS )
   {
     ...
     else
     {
        ...
        if ( allDisabled )
        {
           ...
           popups_add_topmost(p);
           radio_set_out_datarate(DEFAULT_DATARATE_VIDEO);
           log_line("Finished executing start up sequence step: %d", s_StartSequence);
...
void ruby_input_loop(bool bNoKeys)
{
   ...
   //log_line("DEBUG central-hwloop");
   hardware_loop();

   //log_line("DEBUG central-comm");
   if ( 0 != hardware_get_radio_interfaces_count() )
      handle_commands_loop();

   //log_line("DEBUG central-keys");
...
void main_loop_r_central()
{
   ...
   //log_line("DEBUG central-rmrouter");
   
   try_read_messages_from_router(7);

   //log_line("DEBUG central-input");
   ...
   //log_line("DEBUG central-endloop");
}
