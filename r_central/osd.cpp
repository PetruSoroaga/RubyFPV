...
void osd_show_voltage(float x, float y, float voltage, bool bRightAlign)
{
   ...
   if ( g_pdpfct->voltage > 1000 )
   {
      ...
      {
         float voltage = g_pdpfct->voltage/1000.0;
         int cellCount = floor(voltage/4.3)+1;
         if ( cellCount > s_computedOSDCellCount )
         {
            s_computedOSDCellCount = cellCount;
            log_line("Updated battery cell count to %d (for received voltage of: %.2f)", s_computedOSDCellCount, voltage );
         }
      }
   }

   if ( NULL != g_pCurrentModel )
   if ( s_bDebugOSDShowAll || (g_pCurrentModel->osd_params.osd_flags2[s_nCurrentOSDFlagsIndex] & OSD_FLAG_EXT_SHOW_BATTERY_CELLS ) )
   if ( 0 != s_computedOSDCellCount )
   {
      ...
      snprintf(szBuff, 31, "%.2f", 31, voltage/(float)s_computedOSDCellCount);
      ...
      snprintf(szBuff, 31, "(%d cells)", s_computedOSDCellCount);
      if ( bRightAlign )
...
float _osd_show_gps(float x, float y, bool bMultiLine)
{
   ...
   if ( g_pdpfct->gps_fix_type >= GPS_FIX_TYPE_3D_FIX || (( g_TimeNow / 300 ) % 3) != 0 )
   ...
   if ( bMultiLine )
   {
      ...
      sprintf(szBuff, "%.1f", g_pdpfct->hdop/100.0);
   ...
   u16 hdop2 = (((u16)g_pdpfct->extra_info[3])<<8) + ((u16)g_pdpfct->extra_info[4]);
   u8 satelites2 = g_pdpfct->extra_info[1];
   ...
   if ( ((g_pdpfct->extra_info[2] >= GPS_FIX_TYPE_3D_FIX) && (g_pdpfct->extra_info[2] != 0xFF)) || (( g_TimeNow / 300 ) % 3) != 0 )
      g_pRenderEngine->drawIcon(x,y, 0.8*height_text_big/g_pRenderEngine->getAspectRatio(), 0.8*height_text_big, g_idIconGPS);
   x += 1.0*height_text_big/g_pRenderEngine->getAspectRatio();

   if ( (g_pdpfct->extra_info[2] >= GPS_FIX_TYPE_3D_FIX) && (g_pdpfct->extra_info[2] != 0xFF) )
      g_pRenderEngine->drawText(x-0.3*height_text_big, y-0.7*osd_getSpacingV(), height_text*0.4, g_idFontOSDSmall, "3D");

   if ( g_pdpfct->extra_info[1] == 0xFF )
      strcpy(szBuff, "N/A");
   else
      sprintf(szBuff, "%d", g_pdpfct->extra_info[1]);
   w = osd_show_value(x,y, szBuff, g_idFontOSDBig);
   x += w + 0.5*osd_getSpacingH();
...
float osd_show_flight_mode(float x, float y)
{
   ...
   u8 mode = 0;
   if ( NULL != g_pdpfct )
      mode = g_pdpfct->flight_mode & (~FLIGHT_MODE_ARMED);
   ...
   if ( NULL != g_pdpfct && (g_pdpfct->flags & FC_TELE_FLAGS_ARMED) )
   {
...
float osd_show_video_link_mbs(float xPos, float yPos, bool bLeft)
{
   ...
   if ( NULL == g_pPHRTE || NULL == g_pCurrentModel )
      strcpy(szBuff,"0");
   else
      sprintf(szBuff, "%.1f (%.1f)", totalMaxVideo_bps/1000.0/1000.0, g_pPHRTE->downlink_tx_video_bitrate/1000.0/1000.0);
   ...
   if ( g_pCurrentModel->getLinkDataRate(2) > nDataRate )
      nDataRate = g_pCurrentModel->getLinkDataRate(2);

   if ( totalMaxVideo_bps/1000.0/1000.0 >= (float)(nDataRate) * DEFAULT_VIDEO_LINK_MAX_LOAD_PERCENT / 100.0 )
...
float osd_show_armtime(float xPos, float yPos, bool bRender)
{
   ...
   if ( g_pCurrentModel->m_Stats.uCurrentFlightTime > 0 )
   {
      ...
      if ( (g_pCurrentModel->m_Stats.uCurrentFlightTime % 2) == 0 )
      {
         ...
         if ( g_pCurrentModel->osd_params.osd_flags2[s_nCurrentOSDFlagsIndex] & OSD_FLAG_EXT_SHOW_BACKGROUND_ON_TEXTS_ONLY )
...
float osd_show_home(float xPos, float yPos, bool showHeading, float fScale)
{
   ...
   float rel_heading = heading_home - g_pdpfct->heading; //direction arrow needs to point relative to camera/osd/craft
   if ( g_pCurrentModel->osd_params.invert_home_arrow )
...
float osd_show_radio_interface(int iInterfaceIndex, int iLinkNumber, bool bVehicle, float xPos, float yPos, float fScale)
{
   ...
   if ( g_pCurrentModel->osd_params.osd_flags2[s_nCurrentOSDFlagsIndex] & OSD_FLAG_EXT_SHOW_RADIO_LINK_QUALITY_PERCENTAGE )
      bShowPercentages = true;

   if ( g_pCurrentModel->osd_params.osd_flags2[s_nCurrentOSDFlagsIndex] & OSD_FLAG_EXT_SHOW_RADIO_LINK_QUALITY_BARS )
      bShowBars = true;

   if ( (g_pCurrentModel->osd_params.osd_flags2[s_nCurrentOSDFlagsIndex] & OSD_FLAG_EXT_SHOW_RADIO_LINK_INTERFACES_EXTENDED) ||
        (g_pCurrentModel->osd_params.osd_flags[s_nCurrentOSDFlagsIndex] & OSD_FLAG_SHOW_RADIO_INTERFACES_INFO) )
      bShowCardInfo = true;
   ...
   if ( bShowCardInfo )
   {
      if ( bVehicle )
      {
         ...
         int dbm = g_pPHRTE->uplink_rssi_dbm[iLinkNumber]-200;
         if ( dbm < -110 )
         {
   ...
   if ( bVehicle )
   {
      nQuality = g_pPHRTE->uplink_link_quality[iLinkNumber];
      nDataRateVideo = g_pPHRTE->uplink_datarate[iLinkNumber]/2;
...
{
   if ( NULL == g_pPHRTE || NULL == g_pCurrentModel || NULL == g_pSM_RadioStats )
      return 0.0;
...
float osd_show_cpus(float xPos, float yPos, float fScale )
{
   ...
   if ( s_bDebugOSDShowAll || (g_pCurrentModel->osd_params.osd_flags[s_nCurrentOSDFlagsIndex] & OSD_FLAG_SHOW_CPU_INFO ) )
   {
      if ( NULL != g_pPHRTE )
      {
         Preferences* p = get_Preferences();
         if ( p->iUnits == prefUnitsImperial || p->iUnits == prefUnitsFeets )
            sprintf(szBuff, "%d F",  (int)osd_convertTemperature(g_pPHRTE->temperature));
         else
            sprintf(szBuff, "%d C",  (int)osd_convertTemperature(g_pPHRTE->temperature));
         if ( g_pPHRTE->temperature >= 70 )
         if ( (g_TimeNow/500)%2 )
            g_pRenderEngine->setColors(get_Color_IconWarning());
         if ( g_pPHRTE->temperature >= 75 )
         if ( (g_TimeNow/500)%2 )
            g_pRenderEngine->setColors(get_Color_IconError());
      }
      else
      ...
      g_pRenderEngine->drawIcon(xPos-dxIcon, yPos-height_text*0.2, 1.2*height_text/g_pRenderEngine->getAspectRatio(), 1.2*height_text, g_idIconTemperature);
      xPos -= dxIcon;
      xPos -= height_text*0.2;
      ...
      if ( NULL != g_pPHRTE )
         sprintf(szBuff, "%d Mhz",  (int)((float)g_pPHRTE->cpu_mhz));// * 0.953)); // 1024 scalling
      else
         sprintf(szBuff, "- Mhz");
      xPos -= osd_show_value_left(xPos, yPos, szBuff, g_idFontOSDSmall);
      ...
      if ( NULL != g_pPHRTE )
         sprintf(szBuff, "%d %%", g_pPHRTE->cpu_load);
      else
         strcpy(szBuff, "- %%");
      ...
      g_pRenderEngine->drawIcon(xPos-dxIcon, yPos-height_text*0.1, 1.2*height_text/g_pRenderEngine->getAspectRatio(), 1.2*height_text, g_idIconCPU);
      xPos -= dxIcon;
...
float osd_show_gps_pos(float xPos, float yPos, float fScale)
{
   ...
   if ( g_pCurrentModel->osd_params.osd_flags2[s_nCurrentOSDFlagsIndex] & OSD_FLAG_EXT_LAYOUT_LEFT_RIGHT )
      ...
      sprintf(szBuff, "%.6f,", g_stats_fLastPosLat);
   if ( NULL != g_pCurrentModel && ((g_pCurrentModel->osd_params.osd_flags[s_nCurrentOSDFlagsIndex]) & OSD_FLAG_SCRAMBLE_GPS) )
   {
      int index = 0;
...
float osd_show_total_distance(float xPos, float yPos, float fScale)
{
   ...
   {
      if ( _osd_convertKm(g_pdpfct->total_distance/100.0/1000.0) > 1.0 )
         sprintf(szBuff, "%s %u mi", szPrefix, (unsigned long)_osd_convertKm(g_pdpfct->total_distance/100.0/1000.0));
      else
         sprintf(szBuff, "%s %u ft", szPrefix, (unsigned long)_osd_convertMeters(g_pdpfct->total_distance/100));
   }
   else
   {
      if ( _osd_convertKm(g_pdpfct->total_distance/100.0/1000.0) > 1.0 )
         sprintf(szBuff, "%s %u km", szPrefix, (unsigned long)_osd_convertKm(g_pdpfct->total_distance/100.0/1000.0));
      else
         sprintf(szBuff, "%s %u m", szPrefix, (unsigned long)_osd_convertMeters(g_pdpfct->total_distance/100));
   }
   osd_set_colors();
   float w = osd_show_value(xPos,yPos, szBuff, g_idFontOSDSmall);
   return w;
...
void osd_show_recording(bool bShowWhenStopped, float xPos, float yPos)
{
   ...
   {
      if ( !(g_pCurrentModel->osd_params.osd_flags2[s_nCurrentOSDFlagsIndex] & OSD_FLAG_EXT_LAYOUT_LEFT_RIGHT) )
      if ( g_pCurrentModel->osd_params.osd_flags[s_nCurrentOSDFlagsIndex] & OSD_FLAG_SHOW_VIDEO_MODE )
      if ( bShowWhenStopped )
      {
         s_lMemDiskOSDFree = 0;
         ...
         if ( g_bToglleAllOSDOff || g_bToglleOSDOff )
            return;
         if ( (!s_bDebugOSDShowAll) && (!(g_pCurrentModel->osd_params.osd_flags[s_nCurrentOSDFlagsIndex] & OSD_FLAG_SHOW_VIDEO_MODE )) )
            return;
...
void osd_show_signal_bars()
{
   ...
   int position = (g_pCurrentModel->osd_params.osd_flags[s_nCurrentOSDFlagsIndex] & OSD_FLAG_SIGNAL_BARS_MASK)>>14;
...
void osd_show_grid()
{
   ...
   if ( s_bDebugOSDShowAll || (g_pCurrentModel->osd_params.osd_flags3[s_nCurrentOSDFlagsIndex] & OSD_FLAG3_SHOW_GRID_CROSSHAIR) )
   {
   ...
   if ( s_bDebugOSDShowAll || (g_pCurrentModel->osd_params.osd_flags3[s_nCurrentOSDFlagsIndex] & OSD_FLAG3_SHOW_GRID_DIAGONAL) )
   {
      g_pRenderEngine->drawLine(0.001, 0.001, 0.999, 0.999 ); 
      g_pRenderEngine->drawLine(0.999, 0.001, 0.001, 0.999);  
   }

   if ( s_bDebugOSDShowAll || (g_pCurrentModel->osd_params.osd_flags3[s_nCurrentOSDFlagsIndex] & OSD_FLAG3_SHOW_GRID_SQUARES) )
   {
...
float render_osd_voltagesamps(float x, float y)
{
   ...
   if ( s_bDebugOSDShowAll || (g_pCurrentModel->osd_params.osd_flags[s_nCurrentOSDFlagsIndex] & OSD_FLAG_SHOW_BATTERY ) )
   {
      ...
      osd_show_voltage(x,y, g_pdpfct->voltage/1000.0, true);
      if ( bMultiLine )
   ...
   if ( s_bDebugOSDShowAll || (g_pCurrentModel->osd_params.osd_flags[s_nCurrentOSDFlagsIndex] & OSD_FLAG_SHOW_BATTERY ) )
   {
      osd_show_amps(x,y, g_pdpfct->current/1000.0, true);
      if ( bMultiLine )
      {
         x = x0;
         y += osd_getSecondBarHeight();
...
void _render_osd_left_right()
{
   ...
   if ( s_bDebugOSDShowAll || (g_pCurrentModel->osd_params.osd_flags2[s_nCurrentOSDFlagsIndex] & OSD_FLAG_EXT_SHOW_BGBARS) )
      render_bars();

   if ( s_bDebugOSDShowAll || (g_pCurrentModel->osd_params.osd_flags[s_nCurrentOSDFlagsIndex] & OSD_FLAG_SHOW_HID_IN_OSD) )
      osd_show_HID();

   if ( s_bDebugOSDShowAll ||
      (g_pCurrentModel->osd_params.osd_flags3[s_nCurrentOSDFlagsIndex] & OSD_FLAG3_SHOW_GRID_CROSSHAIR) ||
      (g_pCurrentModel->osd_params.osd_flags3[s_nCurrentOSDFlagsIndex] & OSD_FLAG3_SHOW_GRID_DIAGONAL) ||
      (g_pCurrentModel->osd_params.osd_flags3[s_nCurrentOSDFlagsIndex] & OSD_FLAG3_SHOW_GRID_SQUARES) )
      osd_show_grid();

   if ( s_bDebugOSDShowAll || (g_pCurrentModel->osd_params.osd_flags[s_nCurrentOSDFlagsIndex] & OSD_FLAG_SHOW_SIGNAL_BARS) )
   ...
   if ( s_bDebugOSDShowAll || (g_pCurrentModel->osd_params.osd_flags[s_nCurrentOSDFlagsIndex] & OSD_FLAG_SHOW_GPS_INFO ) )
   {
      _osd_show_gps(x,y, false);
      y += height_text_big + vSpacing;
   }
 
   if ( s_bDebugOSDShowAll || (g_pCurrentModel->osd_params.osd_flags2[s_nCurrentOSDFlagsIndex] & OSD_FLAG_EXT_SHOW_GPS_POS ) )
   {
   ...
   if ( bShowAnySpeed || (g_pCurrentModel->osd_params.osd_flags[s_nCurrentOSDFlagsIndex] & OSD_FLAG_SHOW_DISTANCE) )
      y += 1.0*vSpacing;

   if ( s_bDebugOSDShowAll || (g_pCurrentModel->osd_params.osd_flags2[s_nCurrentOSDFlagsIndex] & OSD_FLAG_EXT_SHOW_GROUND_SPEED) )
   {
      strcpy(szBuff,"0");
   ...
   if ( s_bDebugOSDShowAll || (g_pCurrentModel->osd_params.osd_flags2[s_nCurrentOSDFlagsIndex] & OSD_FLAG_EXT_SHOW_AIR_SPEED) )
   {
      strcpy(szBuff,"0");
      if ( link_has_fc_telemetry() )
      {
   ...
   {
      bShowDistance = true;
      sprintf(szBuff, "D: %u", (unsigned int) _osd_convertMeters(g_pdpfct->distance/100.0));
      if ( p->iUnits == prefUnitsImperial || p->iUnits == prefUnitsFeets )
      {
        if ( _osd_convertMeters(g_pdpfct->distance/100.0) >= 10000.0 )
      ...
      {
         if ( _osd_convertMeters(g_pdpfct->distance/100.0) >= 10000 )
        ...
        g_pCurrentModel->vehicle_type == MODEL_TYPE_HELI )
   {
     ...
     if ( link_has_fc_telemetry() )
     {
        if ( g_pCurrentModel->osd_params.osd_flags2[s_nCurrentOSDFlagsIndex] & OSD_FLAG_EXT_SHOW_LOCAL_VERTICAL_SPEED )
     ...
     float alt = _osd_convertMeters(g_pdpfct->altitude_abs/100.0f-1000.0);
     if ( NULL != g_pCurrentModel && g_pCurrentModel->osd_params.altitude_relative )
        alt = _osd_convertMeters(g_pdpfct->altitude/100.0f-1000.0);
     if ( fabs(alt) < 10.0 )
     {
        if ( fabs(alt) < 0.1 )
          alt = 0.0;
   ...
   if ( s_bDebugOSDShowAll || (g_pCurrentModel->osd_params.osd_flags[s_nCurrentOSDFlagsIndex] & OSD_FLAG_SHOW_PITCH ) )
   if ( NULL != g_pCurrentModel )
   if ( g_pCurrentModel->vehicle_type == MODEL_TYPE_GENERIC ||
        g_pCurrentModel->vehicle_type == MODEL_TYPE_DRONE || 
        g_pCurrentModel->vehicle_type == MODEL_TYPE_AIRPLANE ||
   ...
   {
      if ( NULL != g_pdpfct )  
         sprintf(szBuff, "Pitch: %d", (int)(g_pdpfct->pitch/100.0-180.0));
      else
   ...
   if ( s_bDebugOSDShowAll || (g_pCurrentModel->osd_params.osd_flags2[s_nCurrentOSDFlagsIndex] & OSD_FLAG_EXT_SHOW_RC_RSSI ) )
   {
   ...
   /*
   if ( s_bDebugOSDShowAll || (g_pCurrentModel->osd_params.osd_flags[s_nCurrentOSDFlagsIndex] & OSD_FLAG_SHOW_VEHICLE_RADIO_LINKS) )
   {
   ...
   */
   ...
   if ( s_bDebugOSDShowAll || (g_pCurrentModel->osd_params.osd_flags[s_nCurrentOSDFlagsIndex] & OSD_FLAG_SHOW_RADIO_LINKS) || (g_pCurrentModel->osd_params.osd_flags[s_nCurrentOSDFlagsIndex] & OSD_FLAG_SHOW_VEHICLE_RADIO_LINKS) )
      for( int iRadioLink=0; iRadioLink<g_pSM_RadioStats->countRadioLinks; iRadioLink++ )
      {
   ...
   //if ( s_bDebugOSDShowAll || (g_pCurrentModel->osd_params.osd_flags[s_nCurrentOSDFlagsIndex] & OSD_FLAG_SHOW_RADIO_INTERFACES ) )
   //{
   ...
   if ( s_bDebugOSDShowAll || (g_pCurrentModel->osd_params.osd_flags[s_nCurrentOSDFlagsIndex] & OSD_FLAG_SHOW_VIDEO_MBPS ) )
   {
      osd_show_video_link_mbs(x,y, true);
      y += height_text_big + vSpacing;
   }

   if ( s_bDebugOSDShowAll || (g_pCurrentModel->osd_params.osd_flags[s_nCurrentOSDFlagsIndex] & OSD_FLAG_SHOW_VIDEO_MODE_EXTENDED ) )
   {
...
void osd_debug()
{
   g_pdpfct->flags = FC_TELE_FLAGS_ARMED | FC_TELE_FLAGS_HAS_ATTITUDE;
   g_pdpfct->flight_mode = FLIGHT_MODE_STAB | FLIGHT_MODE_ARMED;
   g_pdpfct->gps_fix_type = GPS_FIX_TYPE_3D_FIX;
   g_pdpfct->hdop = 111;

   if ( s_RenderCount < 2 || g_pdpfct->hspeed < 5000 )
   {
      g_pdpfct->altitude = 101000;
      g_pdpfct->altitude_abs = 101000;
      g_pdpfct->vspeed = 101000;
      g_pdpfct->hspeed = 101000;
      g_pdpfct->heading = 2;
      g_pdpfct->roll = 180*100;
      g_pdpfct->pitch = 180*100;
   }

   if ( s_RenderCount % 10 )
   {
      // Current pos: 47.14378586227096, 27.583722513468402
      g_pdpfct->latitude = 47.136136 * 10000000 + 20000 * sin(s_RenderCount/50.0);
      g_pdpfct->longitude = 27.5777667 * 10000000 + 20000 * cos(s_RenderCount/50.0);
      g_pdpfct->flags |= FC_TELE_FLAGS_POS_CURRENT;
      g_pdpfct->distance = 100 * distance_meters_between( g_stats_fHomeLat, g_stats_fHomeLon, g_pdpfct->latitude/10000000.0, g_pdpfct->longitude/10000000.0 );
   }
   else
   {
      // Home:  47.13607918623999, 27.577757896625428
      g_pdpfct->latitude = 47.136136 * 10000000;
      g_pdpfct->longitude = 27.5777667 * 10000000;
      g_pdpfct->flags |= FC_TELE_FLAGS_POS_HOME;     
   }

   u32 tm = g_TimeNow;

   g_pdpfct->satelites++;
   if ( g_pdpfct->satelites > 24 )
      g_pdpfct->satelites = 0;

   g_pdpfct->throttle++;
   if ( g_pdpfct->throttle > 100 )
      g_pdpfct->throttle = 0;

   //g_pdpfct->heading = 120;
   g_pdpfct->heading+=2;
   if ( g_pdpfct->heading > 360 )
      g_pdpfct->heading -= 360;

   g_pdpfct->voltage+=50;
   if ( g_pdpfct->voltage > 16000 )
      g_pdpfct->voltage = 0;

   g_pdpfct->altitude_abs+=20;
   g_pdpfct->altitude+=20;
   if ( g_pdpfct->altitude > 105240 )
      g_pdpfct->altitude = 99580;

   g_pdpfct->hspeed+=10;
   if ( g_pdpfct->hspeed > 104000 )
      g_pdpfct->hspeed = 100000;

   g_pdpfct->vspeed+=10;
   if ( g_pdpfct->vspeed > 102540 )
      g_pdpfct->vspeed = 98770;

   g_pdpfct->roll+=20;
   if ( g_pdpfct->roll > 230*100 )
      g_pdpfct->roll = 130*100;

   g_pdpfct->pitch+=20;
   if ( g_pdpfct->pitch > 230*100 )
      g_pdpfct->pitch = 130*100;
}
...
void osd_render_elements()
{
   if ( NULL != g_pCurrentModel && ( 0 < g_pCurrentModel->iGPSCount) && NULL != g_pdpfct )
   {
      if ( g_pdpfct->flags & FC_TELE_FLAGS_POS_CURRENT )
      {
         g_stats_fLastPosLat = g_pdpfct->latitude/10000000.0f;
         g_stats_fLastPosLon = g_pdpfct->longitude/10000000.0f;
      }
      if ( g_pdpfct->flags & FC_TELE_FLAGS_POS_HOME )
      {
         if ( ! g_stats_bHomeSet )
            warnings_add("Home Position Aquired and Stored");
         g_stats_bHomeSet = true;
         g_stats_fHomeLat = g_pdpfct->latitude/10000000.0f;
         g_stats_fHomeLon = g_pdpfct->longitude/10000000.0f;
      }
   }
   ...
   if ( !(g_pCurrentModel->osd_params.osd_flags2[s_nCurrentOSDFlagsIndex] & OSD_FLAG_EXT_LAYOUT_ENABLED) )
      return;
   ...
   if ( s_bDebugOSDShowAll || (g_pCurrentModel->osd_params.osd_flags[s_nCurrentOSDFlagsIndex] & OSD_FLAG_SHOW_TIME ) )
   {
      if ( !(g_pCurrentModel->osd_params.osd_flags[s_nCurrentOSDFlagsIndex] & OSD_FLAG_SHOW_TIME_LOWER) )
        osd_show_armtime(x,y, true);
   }
   ...
   */
   ...
   if ( s_bDebugOSDShowAll || (g_pCurrentModel->osd_params.osd_flags2[s_nCurrentOSDFlagsIndex] & OSD_FLAG_EXT_SHOW_GROUND_SPEED) || (g_pCurrentModel->osd_params.osd_flags2[s_nCurrentOSDFlagsIndex] & OSD_FLAG_EXT_SHOW_AIR_SPEED) || (g_pCurrentModel->osd_params.osd_flags[s_nCurrentOSDFlagsIndex] & OSD_FLAG_SHOW_DISTANCE) )
   {
        ...
        g_pCurrentModel->vehicle_type == MODEL_TYPE_HELI )
   {
     ...
     //if ( ( g_pdpfct->vspeed/100.0f - 1000.0 ) > 0.001 )
     //   g_pRenderEngine->drawText(x, y,height_text*0.95, g_idFontOSDIcons, "");
     //else if ( ( g_pdpfct->vspeed/100.0f - 1000.0 ) < -0.001 )
     //   g_pRenderEngine->drawText(x, y,height_text*0.95, g_idFontOSDIcons, "");
     //else
     ...
     if ( link_has_fc_telemetry() )
     {
        float vSpeed = 0.0;
        if ( g_pCurrentModel->osd_params.osd_flags2[s_nCurrentOSDFlagsIndex] & OSD_FLAG_EXT_SHOW_LOCAL_VERTICAL_SPEED )
           vSpeed = _osd_convertMeters(g_pdpfct->vspeed/100.0f-1000.0);
        else
           vSpeed = _osd_convertMeters(g_pdpfct->vspeed/100.0f-1000.0);
        if ( fabs(vSpeed) < 0.1 )
           vSpeed = 0.0;
   ...
   if ( s_bDebugOSDShowAll || (g_pCurrentModel->osd_params.osd_flags3[s_nCurrentOSDFlagsIndex] & OSD_FLAG3_SHOW_WIND ) )
   if ( NULL != g_pdpfct )
   {
      ...
      u16 uDir = ((u16)g_pdpfct->extra_info[7] << 8) | g_pdpfct->extra_info[8];
      u16 uSpeed = ((u16)g_pdpfct->extra_info[9] << 8) | g_pdpfct->extra_info[10];
      if ( 0 == uDir )
         strcpy(szBuff, "H: N/A");
      else
      ...
      x += osd_show_value(x,y+height_text_small*0.8, szBuff, g_idFontOSD);
      x += osd_getSpacingH();
   }
   ...
   if ( s_bDebugOSDShowAll || (g_pCurrentModel->osd_params.osd_flags[s_nCurrentOSDFlagsIndex] & OSD_FLAG_SHOW_TIME ) )
   {
      ...
      {
         x -= osd_getSpacingH()*0.5;
         float fW = osd_show_armtime(x,y,false);
         x -= osd_show_armtime(x-fW,y, true);
         x -= osd_getSpacingH();
      }
   }
   ...
   {
      if ( NULL != g_pdpfct )  
         sprintf(szBuff, "%d", (int)(g_pdpfct->pitch/100.0-180.0));
      else
         sprintf(szBuff, "-");
   ...
   if ( s_bDebugOSDShowAll || (g_pCurrentModel->osd_params.osd_flags3[s_nCurrentOSDFlagsIndex] & OSD_FLAG3_SHOW_FC_TEMPERATURE ) )
   {
      if ( NULL == g_pdpfct || 0 == g_pdpfct->temperature )
         strcpy(szBuff, "N/A");
      else
      {
         if ( p->iUnits == prefUnitsImperial || p->iUnits == prefUnitsFeets )
            sprintf(szBuff, "%d F", (int)osd_convertTemperature(g_pdpfct->temperature-100));
         else
            sprintf(szBuff, "%d C", (int)osd_convertTemperature(g_pdpfct->temperature-100));
      }
...
void osd_render_instruments()
{
   ...
   if ( g_pdpfct->flags & FC_TELE_FLAGS_HAS_ATTITUDE )
      osd_show_ahi(g_pdpfct->roll/100.0-180.0, g_pdpfct->pitch/100.0-180.0);
   else
      osd_show_ahi(0,0);
...
{
   if ( NULL == g_pCurrentModel || NULL == g_pPHRTE || g_pCurrentModel->is_spectator )
      return;
...
void osd_render_all()
{
   ...
   // Wrong model ?
   if ( pairing_isStarted() && pairing_isReceiving() && g_pPHRTE->vehicle_id != g_pCurrentModel->vehicle_id )
      return;
   ...
   u32 tr = ((g_pCurrentModel->osd_params.osd_preferences[s_nCurrentOSDFlagsIndex])>>8) & 0xFF;
   osd_set_transparency((int)tr);

   u32 scale = g_pCurrentModel->osd_params.osd_preferences[s_nCurrentOSDFlagsIndex] & 0xFF;
   osd_setScaleOSD((int)scale);

   scale = (g_pCurrentModel->osd_params.osd_preferences[s_nCurrentOSDFlagsIndex]>>16) & 0x0F;
