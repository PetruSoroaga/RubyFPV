...
:Menu(MENU_ID_CONTROLLER, "Controller Settings", NULL)
{
   ...
   m_IndexVideo = addMenuItem(new MenuItem("Video Output", "Change the Video Output Settings (HDMI, USB Tethering)"));
...
void MenuController::onReturnFromChild(int returnValue)
{
   ...
   if ( 5 == m_iConfirmationId )
   {
      ...
      onNewVehicleActive();
      pairing_start();
      log_line("Completed the failed update procedure after user confirmed it.");
      */
      return;
   }
...
void MenuController::onSelectItem()
{
   ...
   /*
   if ( m_IndexShowCPUInfo == m_SelectedIndex )
   {
      pp->iShowControllerCPUInfo = 1 - pp->iShowControllerCPUInfo;
      save_Preferences();
      apply_Preferences();
      valuesToUI();
      return;
   ...
   if ( m_IndexShowVoltage == m_SelectedIndex )
   {
      pCS->iShowVoltage = m_pItemsSelect[2]->getSelectedIndex();
...
void MenuController::updateSoftware()
{
   ...
   if ( iResult < 0 )
   {
      ...
      if ( iResult == -1 )
         pMC = new MenuConfirmation("Update Infp", "No update archive file found on USB memory stick. No update done.",m_iConfirmationId, true);
      else if ( iResult == -2 )
