...
:Menu(MENU_ID_VEHICLE_RADIO_LINK, "Vehicle Radio Link Parameters", NULL)
{
   ...
   log_line("Opening menu for radio link %d parameters...", m_iRadioLink);
   ...
   for( int ch=0; ch<m_SupportedChannelsCount; ch++ )
   {
      ...
      sprintf(szBuff,"%d Mhz", m_SupportedChannels[ch]);
...
void MenuVehicleRadioLink::valuesToUI()
{
   ...
   for( int ch=0; ch<m_SupportedChannelsCount; ch++ )
   {
      if ( g_pCurrentModel->radioLinksParams.link_frequency[m_iRadioLink] == m_SupportedChannels[ch])
         break;
      selectedIndex++;
   }
...
void MenuVehicleRadioLink::onSelectItem()
{
   ...
   if ( -1 == iRadioInterfaceId )
      ...
      if ( m_IndexFrequency == m_SelectedIndex )
      {
         ...
         int freq = m_SupportedChannels[index];
      ...
      if ( m_IndexUsage == m_SelectedIndex )
      {
         ...
         nicFlags[m_iRadioLink] = nicFlags[m_iRadioLink] & (~(RADIO_HW_CAPABILITY_FLAG_CAN_USE_FOR_VIDEO|RADIO_HW_CAPABILITY_FLAG_CAN_USE_FOR_DATA|RADIO_HW_CAPABILITY_FLAG_CAN_USE_FOR_RELAY));
