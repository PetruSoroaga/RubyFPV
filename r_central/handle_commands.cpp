   ...
   switch ( s_CommandType )
   {
      ...
      case COMMAND_ID_GET_CURRENT_VIDEO_CONFIG:
         ...
         for( int i=0; i<g_pCurrentModel->radioInterfacesParams.interfaces_count; i++ )
         {
            ...
            s_pMenuVehicleHWInfo->addTopLine(szBuff);
            sprintf(szBuff, ". . . currently at %d Mhz, supported bands: ", g_pCurrentModel->radioInterfacesParams.interface_current_frequency[i]);
            ...
            if ( g_pCurrentModel->radioInterfacesParams.interface_capabilities_flags[i] & RADIO_HW_CAPABILITY_FLAG_CAN_USE_FOR_DATA )
               strcat(szBuff, "Data, ");
            if ( g_pCurrentModel->radioInterfacesParams.interface_capabilities_flags[i] & RADIO_HW_CAPABILITY_FLAG_CAN_USE_FOR_RELAY )
               strcat(szBuff, "Relay");
            if ( !(g_pCurrentModel->radioInterfacesParams.interface_capabilities_flags[i] & RADIO_HW_CAPABILITY_FLAG_CAN_USE_FOR_RELAY) )
               strcat(szBuff, "No relay");
            s_pMenuVehicleHWInfo->addTopLine(szBuff);
         }
     ...
     case COMMAND_ID_GET_USB_INFO2:
     {
      ...
      case COMMAND_ID_SET_RADIO_LINK_FREQUENCY:
         tmpLink = (s_CommandParam>>16);
         tmpFreq = (s_CommandParam & 0xFFFF);
         log_line("Received response Ok from vehicle to the link frequency change. Vehicle radio link %d new freq: %d Mhz", tmpLink+1, tmpFreq);
         if ( tmpLink >= 0 && tmpLink < g_pCurrentModel->radioLinksParams.links_count )
         {
            ...
            data[0] = tmpLink;
            data[1] = tmpFreq;
            send_control_message_to_router_and_data(PACKET_TYPE_LOCAL_LINK_FREQUENCY_CHANGED, (u8*)(&data[0]), 2*sizeof(int));
         }
         break;
      ...
      case COMMAND_ID_SET_OSD_PARAMS:
         {
            ...
            saveAsCurrentModel(g_pCurrentModel);
            send_control_message_to_router(PACKET_TYPE_LOCAL_CONTROL_MODEL_CHANGED, PACKET_COMPONENT_RUBY);
            break;
         }
...
void handle_commands_loop()
{
   ...
   if ( pPHCR->origin_command_counter != s_CommandCounter )
   {
      log_line_commands( "[CommandsTh] [Recv] Ignore response from vId %u, cmd resp nb. %d, origin cmd nb. %d, origin retry %d, type [%s], flags %d, extra info len. %d]", pPH->vehicle_id_src, pPHCR->response_counter, pPHCR->origin_command_counter, pPHCR->origin_command_resend_counter, commands_get_description(pPHCR->origin_command_type), pPHCR->command_response_flags, s_CommandReplyLength-sizeof(t_packet_header)-sizeof(t_packet_header_command_response));
      return;
   }
