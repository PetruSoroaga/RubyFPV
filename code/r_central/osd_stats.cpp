...
float osd_render_stats_radio_links( float xPos, float yPos, const char* szTitle, shared_mem_radio_stats* pStats, float scale)
{
...

   {
      ...
      if ( (g_VehiclesRuntimeInfo[g_iCurrentOSDVehicleRuntimeInfoIndex].bGotRubyTelemetryInfo) && (g_VehiclesRuntimeInfo[g_iCurrentOSDVehicleRuntimeInfoIndex].headerRubyTelemetryExtended.txTimePerSec > DEFAULT_TX_TIME_OVERLOAD) )
...
float osd_render_stats_video_decode_get_height(int iDeveloperMode, bool bIsSnapshot, shared_mem_radio_stats* pSM_RadioStats, shared_mem_video_decode_stats* pSM_VideoStats, shared_mem_video_decode_stats_history* pSM_VideoHistoryStats, shared_mem_controller_retransmissions_stats* pSM_ControllerRetransmissionsStats, float scale)
{
   ...
   // Compact stats
   if ( (!bIsSnapshot) && (g_pCurrentModel->osd_params.osd_flags2[g_iCurrentOSDVehicleLayout] & OSD_FLAG_EXT_SHOW_COMPACT_VIDEO_DECODE_STATS) )
   {
      // History graph
   ...
   if ( ! bIsSnapshot )
   if ( (g_pCurrentModel->osd_params.osd_flags2[g_iCurrentOSDVehicleLayout]) & OSD_FLAG_EXT_SHOW_ADAPTIVE_VIDEO_GRAPH )
   {
...
float osd_render_stats_video_decode_get_width(int iDeveloperMode, bool bIsSnapshot, shared_mem_radio_stats* pSM_RadioStats, shared_mem_video_decode_stats* pSM_VideoStats, shared_mem_video_decode_stats_history* pSM_VideoHistoryStats, shared_mem_controller_retransmissions_stats* pSM_ControllerRetransmissionsStats, float scale)
{
   ...
   float width = g_pRenderEngine->textWidth(0.0, s_idFontStats, "AAAAAAAA AAAAAAAA AAAAAAAA A");
   width += 2.0*s_fOSDStatsMargin/g_pRenderEngine->getAspectRatio();
   if ( NULL != g_pCurrentModel &&  ((g_pCurrentModel->osd_params.osd_flags[g_iCurrentOSDVehicleLayout]) & OSD_FLAG_EXTENDED_VIDEO_DECODE_HISTORY) )
      width *= 1.55;
   return width;
}
...
float osd_render_stats_video_decode(float xPos, float yPos, int iDeveloperMode, bool bIsSnapshot, shared_mem_radio_stats* pSM_RadioStats, shared_mem_video_decode_stats* pSM_VideoStats, shared_mem_video_decode_stats_history* pSM_VideoHistoryStats, shared_mem_controller_retransmissions_stats* pSM_ControllerRetransmissionsStats, float scale)
{
   ...
   if ( ! bIsSnapshot )
   {
      ...
      if ( bMustTakeSnapshot )
      {
         ...
         memcpy( &s_Snapshot_PHRTE_Retransmissions, &(g_VehiclesRuntimeInfo[g_iCurrentOSDVehicleRuntimeInfoIndex].headerRubyTelemetryExtraInfoRetransmissions), sizeof(t_packet_header_ruby_telemetry_extended_extra_info_retransmissions));
         }
      }
   }
   ...
   if ( (!bIsSnapshot) && (g_pCurrentModel->osd_params.osd_flags2[g_iCurrentOSDVehicleLayout] & OSD_FLAG_EXT_SHOW_COMPACT_VIDEO_DECODE_STATS) )
   {
      ...
      if ( iDeveloperMode )
      {
         ...
         strcpy(szBuff, "N/A");
         int percent = 0;
         if ( (NULL != pSM_ControllerRetransmissionsStats) && g_VehiclesRuntimeInfo[g_iCurrentOSDVehicleRuntimeInfoIndex].bGotRubyTelemetryExtraInfoRetransmissions )
         {
...
float osd_render_stats_rc(float xPos, float yPos, float scale)
{
   ...
   if ( NULL == g_pCurrentModel || (! g_VehiclesRuntimeInfo[g_iCurrentOSDVehicleRuntimeInfoIndex].bGotRubyTelemetryInfo) || NULL == g_pPHDownstreamInfoRC )
   {
...
float osd_render_stats_flight_end(float scale)
{
   ...
   if ( NULL == g_pCurrentModel || (!g_VehiclesRuntimeInfo[g_iCurrentOSDVehicleRuntimeInfoIndex].bGotFCTelemetry) )
   {
      g_pRenderEngine->setGlobalAlfa(fAlpha);
      return height;
   }
...
float osd_render_stats_dev(float xPos, float yPos, float scale)
{
   ...
   if ( NULL != g_pSM_RadioStats )
   {
      ...
      {
         if ( (0 == g_pSM_RadioStats->radio_links[i].linkDelayRoundtripMs) || (g_pSM_RadioStats->radio_links[i].linkDelayRoundtripMsLastTime == 0) || (g_pSM_RadioStats->radio_links[i].linkDelayRoundtripMsLastTime == MAX_U32) || (g_pSM_RadioStats->radio_links[i].linkDelayRoundtripMsLastTime < g_TimeNow-1000) )
            continue;
...
void _osd_render_stats_panels_horizontal()
{
   ...
   if ( pCS->iDeveloperMode || s_bDebugStatsShowAll )
   if ( NULL != g_pCurrentModel && (g_pCurrentModel->osd_params.osd_flags3[g_iCurrentOSDVehicleLayout] & OSD_FLAG3_SHOW_VIDEO_BITRATE_HISTORY) )
   {
   ...
   if ( s_bDebugStatsShowAll || (g_pCurrentModel->osd_params.osd_flags2[g_iCurrentOSDVehicleLayout] & OSD_FLAG_EXT_SHOW_STATS_VIDEO) )
   {
   ...
   if ( s_bDebugStatsShowAll || (g_pCurrentModel->osd_params.osd_flags2[g_iCurrentOSDVehicleLayout] & OSD_FLAG_EXT_SHOW_STATS_RADIO_LINKS) )
   if ( NULL != g_pSM_RadioStats )
   {
   ...
   if ( s_bDebugStatsShowAll || (g_pCurrentModel->osd_params.osd_flags2[g_iCurrentOSDVehicleLayout] & OSD_FLAG_EXT_SHOW_STATS_RADIO_INTERFACES) )
   if ( NULL != g_pSM_RadioStats )
   {
   ...
   if ( s_bDebugStatsShowAll || (g_pCurrentModel->osd_params.osd_flags[g_iCurrentOSDVehicleLayout] & OSD_FLAG_SHOW_EFFICIENCY_STATS) )
   {
   ...
   if ( s_bDebugStatsShowAll || (g_pCurrentModel->osd_params.osd_flags2[g_iCurrentOSDVehicleLayout] & OSD_FLAG_EXT_SHOW_STATS_RC) )
   {
...
void _osd_stats_autoarange_left( int iCountArranged, int iCurrentColumn )
{
   ...
   {
      if ( fColumnCurrentHeight + s_iOSDStatsBoundingBoxesH[i] <= 1.0 - s_fOSDStatsMarginV - s_fOSDStatsMarginVTop )
      {
...
void _osd_stats_autoarange_right( int iCountArranged, int iCurrentColumn )
{
   ...
   for( int i=iCountArranged; i<s_iCountOSDStatsBoundingBoxes; i++ )
   {
      if ( fColumnCurrentHeight + s_iOSDStatsBoundingBoxesH[i] <= 1.0 - 1.4*(s_fOSDStatsMarginV+s_fOSDStatsMarginVTop) )
      {
         // Move it to sorted position
         _osd_stats_swap_pannels(iCountArranged, i);
         s_iOSDStatsBoundingBoxesX[iCountArranged] = fCurrentColumnX - s_iOSDStatsBoundingBoxesW[iCountArranged];
...
void _osd_stats_autoarange_top()
{
   ...
   {
      if ( s_iOSDStatsBoundingBoxesH[i] >= 1.0 - s_fOSDStatsMarginV - s_fOSDStatsMarginVTop - s_fOSDStatsWindowsMinimBoxHeight - s_fOSDStatsSpacingV )
      {
         // Move it to sorted position
         _osd_stats_swap_pannels(iCountArranged, i);
...
void osd_render_stats_panels()
{
   ...
   s_fOSDStatsMarginV = osd_getMarginY() + osd_getBarHeight() + osd_getSecondBarHeight() + 0.01;
   s_fOSDStatsMarginVTop = s_fOSDStatsMarginV;
   if ( (g_pCurrentModel->osd_params.osd_flags[g_pCurrentModel->osd_params.layout] & OSD_FLAG_SHOW_CPU_INFO ) ||
   ...
   if ( g_pCurrentModel->osd_params.osd_flags3[g_iCurrentOSDVehicleLayout] & OSD_FLAG3_SHOW_CONTROLLER_ADAPTIVE_VIDEO_INFO )
   ...
   if ( s_bDebugStatsShowAll || (g_pCurrentModel->osd_params.osd_flags[g_iCurrentOSDVehicleLayout] & OSD_FLAG_SHOW_STATS_VIDEO_INFO) )
   {
   ...
   if ( g_pCurrentModel->osd_params.osd_preferences[g_iCurrentOSDVehicleLayout] & OSD_PREFERENCES_BIT_FLAG_ARANGE_STATS_WINDOWS_LEFT )
      _osd_stats_autoarange_left(0, 0);
   else if ( g_pCurrentModel->osd_params.osd_preferences[g_iCurrentOSDVehicleLayout] & OSD_PREFERENCES_BIT_FLAG_ARANGE_STATS_WINDOWS_RIGHT )
      _osd_stats_autoarange_right(0, 0);
   else if ( g_pCurrentModel->osd_params.osd_preferences[g_iCurrentOSDVehicleLayout] & OSD_PREFERENCES_BIT_FLAG_ARANGE_STATS_WINDOWS_BOTTOM )
