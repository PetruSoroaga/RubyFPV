...
void MenuVehicleRCFailsafe::Render()
{
   ...
   for( int i=0; i<m_ItemsCount; i++ )
   {
      ...
      if ( (i % m_iColumnsCount) == 0 )
         m_pMenuItems[i]->Render(m_RenderXPos + MENU_MARGINS*menu_getScaleMenus()/g_pRenderEngine->getAspectRatio(), y, i == m_SelectedIndex, fSelWidth*2.0);
      else
      {

         float dx = fSelWidth*1.7 + 0.026*menu_getScaleMenus();
         if ( (i % m_iColumnsCount) == 2 )
            dx += fSelWidth*3.2;
         m_pMenuItems[i]->RenderCondensed(m_RenderXPos + MENU_MARGINS*menu_getScaleMenus()/g_pRenderEngine->getAspectRatio() + dx, y, i == m_SelectedIndex, 0);
      }
