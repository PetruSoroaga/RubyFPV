   ...
   if ( g_pCurrentModel->rc_params.inputType == RC_INPUT_TYPE_USB )
   {
      ...
      if ( NULL == pJoy )
      {
         for( int i=0; i<g_pCurrentModel->rc_params.channelsCount; i++ )
            m_CurrentRCValues[i] = compute_controller_rc_value(g_pCurrentModel, i, m_CurrentRCValues[i], g_pSM_RCIn, NULL, NULL, miliSec);
         return false;
      }
...
{
   char szBuff[128];
   float height_text = g_pRenderEngine->textHeight(MENU_FONT_SIZE_TITLE, g_idFontMenu);
   float height_text_small = g_pRenderEngine->textHeight(MENU_FONT_SIZE_TITLE, g_idFontMenuSmall);
   float fMarginY = MENU_MARGINS*menu_getScaleMenus();
   float fMarginX = fMarginY/g_pRenderEngine->getAspectRatio();

   float fItemSpacing = 1.2 * MENU_ITEM_SPACING * menu_getScaleMenus()*MENU_FONT_SIZE_ITEMS;

   float xPos = m_RenderXPos + m_RenderWidth + 0.02;
   float yPos = m_RenderYPos + 0.02;
   float fWidth = 0.3 * menu_getScaleMenus();
   float fHeight = 2.0*fMarginY + height_text*5.5 + g_pCurrentModel->rc_params.channelsCount * (height_text + fItemSpacing);

   g_pRenderEngine->setColors(get_Color_PopupBg());
   g_pRenderEngine->setStroke(get_Color_PopupBorder());

   g_pRenderEngine->drawRoundRect(xPos, yPos, fWidth, fHeight, 0.01);

   g_pRenderEngine->setColors(get_Color_MenuText());
   
   float y = yPos + fMarginY;
   xPos += fMarginX;
   fWidth -= fMarginX;
   ...
   float x0 = xPos;
   float x1 = xPos + 0.07*menu_getScaleMenus();
   float x2 = xPos + 0.15*menu_getScaleMenus();
   float x3 = xPos + 0.23*menu_getScaleMenus();
   g_pRenderEngine->drawText(x0, y, 0, g_idFontMenu, "Channel");
   g_pRenderEngine->drawText(x1, y, 0, g_idFontMenu, "Live Value");
   g_pRenderEngine->drawText(x2-0.01*menu_getScaleMenus(), y, 0, g_idFontMenu, "FailSafe Type");
   if ( g_pCurrentModel->rc_params.inputType != RC_INPUT_TYPE_RC_IN_SBUS_IBUS )
      g_pRenderEngine->drawText(x3, y, 0, g_idFontMenu, "Expo");

   y += height_text*1.8;
   g_pRenderEngine->drawLine(x0, y-height_text*0.4, xPos+fWidth-fMarginX, y-height_text*0.4);
   ...
   for( int ch=0; ch<g_pCurrentModel->rc_params.channelsCount; ch++ )
   {
      ...
      g_pRenderEngine->drawText(x0, y, 0, g_idFontMenu, szBuff);
