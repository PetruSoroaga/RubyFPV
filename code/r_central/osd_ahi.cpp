...
void osd_show_ahi_heading(float yTop, float fWidth)
{
   ...
   if ( g_VehiclesRuntimeInfo[g_iCurrentOSDVehicleRuntimeInfoIndex].bGotFCTelemetry && link_has_fc_telemetry() )
      heading = g_VehiclesRuntimeInfo[g_iCurrentOSDVehicleRuntimeInfoIndex].headerFCTelemetry.heading;
...
void osd_ahi_detailed_show_main_panels_info(float roll, float pitch)
{
   ...
   strcpy(szBuff,"0");
   float fUISpeed = 0.0;
   if ( g_pCurrentModel->osd_params.osd_flags[g_iCurrentOSDVehicleLayout] & OSD_FLAG_AIR_SPEED_MAIN )
   {
      float fSpeed = 0.0;
      u32 tmp32 = 0;
      if ( g_VehiclesRuntimeInfo[g_iCurrentOSDVehicleRuntimeInfoIndex].bGotFCTelemetry )
      {
        tmp32 = g_VehiclesRuntimeInfo[g_iCurrentOSDVehicleRuntimeInfoIndex].headerFCTelemetry.aspeed;
   ...
   else if ( s_ahi_show_altitude && (NULL != g_pCurrentModel) && link_has_fc_telemetry() && g_VehiclesRuntimeInfo[g_iCurrentOSDVehicleRuntimeInfoIndex].bGotFCTelemetry && 0 != g_VehiclesRuntimeInfo[g_iCurrentOSDVehicleRuntimeInfoIndex].headerFCTelemetry.hspeed)
   {
      float fSpeed = g_VehiclesRuntimeInfo[g_iCurrentOSDVehicleRuntimeInfoIndex].headerFCTelemetry.hspeed/100.0f-1000.0f;
   ...
   if ( NULL != g_pCurrentModel && link_has_fc_telemetry() && g_VehiclesRuntimeInfo[g_iCurrentOSDVehicleRuntimeInfoIndex].bGotFCTelemetry && 0 != g_VehiclesRuntimeInfo[g_iCurrentOSDVehicleRuntimeInfoIndex].headerFCTelemetry.altitude_abs )
   {
      float fAltitude = g_VehiclesRuntimeInfo[g_iCurrentOSDVehicleRuntimeInfoIndex].headerFCTelemetry.altitude_abs/100.0f-1000.0;
      if ( NULL != g_pCurrentModel && g_pCurrentModel->osd_params.altitude_relative && 0 != g_VehiclesRuntimeInfo[g_iCurrentOSDVehicleRuntimeInfoIndex].headerFCTelemetry.altitude )
         fAltitude = g_VehiclesRuntimeInfo[g_iCurrentOSDVehicleRuntimeInfoIndex].headerFCTelemetry.altitude/100.0f-1000.0;
...
void osd_ahi_detailed_show_auxiliary_info(float roll, float pitch)
{
   ...
   {
      strcpy(szBuff,"GS: 0");
      if ( NULL != g_pCurrentModel && link_has_fc_telemetry() && g_VehiclesRuntimeInfo[g_iCurrentOSDVehicleRuntimeInfoIndex].bGotFCTelemetry && 0 != g_VehiclesRuntimeInfo[g_iCurrentOSDVehicleRuntimeInfoIndex].headerFCTelemetry.hspeed)
      {
         float fSpeed = g_VehiclesRuntimeInfo[g_iCurrentOSDVehicleRuntimeInfoIndex].headerFCTelemetry.hspeed/100.0f-1000.0f;
   ...
   if ( s_bDebugAHIShowAll || (g_pCurrentModel->osd_params.instruments_flags[g_iCurrentOSDVehicleLayout] & INSTRUMENTS_FLAG_SHOW_ALTGRAPH) )
      return;
...
void osd_ahi_show_altgraph()
{
   ...
   if ( g_TimeNow > s_ahi_altitude_speed_last_time + 100 )
   {
      ...
      if ( link_has_fc_telemetry() && g_VehiclesRuntimeInfo[g_iCurrentOSDVehicleRuntimeInfoIndex].bGotFCTelemetry )
         s_ahi_altitude_speed_history[0] = g_VehiclesRuntimeInfo[g_iCurrentOSDVehicleRuntimeInfoIndex].headerFCTelemetry.vspeed/100.0f-1000.0;
...
void osd_show_ahi(float roll, float pitch)
{
   ...
   if ( g_pCurrentModel->vehicle_type == MODEL_TYPE_CAR ||
        g_pCurrentModel->vehicle_type == MODEL_TYPE_BOAT ||
        g_pCurrentModel->vehicle_type == MODEL_TYPE_ROBOT )
...
*/
   ...
   if ( s_bDebugAHIShowAll || (g_pCurrentModel->osd_params.instruments_flags[g_iCurrentOSDVehicleLayout] & INSTRUMENTS_FLAG_SHOW_HEADING) )
   {
   ...
   /*
   if ( s_bDebugAHIShowAll || (g_pCurrentModel->osd_params.instruments_flags[g_iCurrentOSDVehicleLayout] & INSTRUMENTS_FLAG_SHOW_HORIZONT) )
   {
   ...
   */
   ...
   if ( s_bDebugAHIShowAll || (g_pCurrentModel->osd_params.instruments_flags[g_iCurrentOSDVehicleLayout] & INSTRUMENTS_FLAG_SHOW_SPEEDALT) )
   {
      osd_ahi_set_color(COLOR_OSD_AHI_LINE_BORDER, 2.0*s_ShadowThicknessPx + s_LineThicknessPx);
      osd_ahi_draw_panels();     
      osd_ahi_set_color(COLOR_OSD_AHI_LINE_FG, s_LineThicknessPx);
