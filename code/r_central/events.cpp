...
{
   log_line("[Event]: Handing event Main Vehicle Changed...");
   ...
   log_line("[Event]: Handing event Main Vehicle Changed. Done.");
}
...
void onEventReboot()
{
   ...
   log_line("[Event]: Handling event Reboot. Done.");
}
...
void onEventBeforePairing()
{
   ...
   g_VehiclesRuntimeInfo[0].bGotRubyTelemetryInfo = false;
   g_VehiclesRuntimeInfo[0].bGotRubyTelemetryExtraInfo = false;
   g_VehiclesRuntimeInfo[0].bGotRubyTelemetryExtraInfoRetransmissions = false;
   g_VehiclesRuntimeInfo[0].uTimeLastRecvRubyTelemetry = 0;

   g_VehiclesRuntimeInfo[0].bGotFCTelemetry = false;
   g_VehiclesRuntimeInfo[0].bGotFCTelemetryExtra = false;
   g_VehiclesRuntimeInfo[0].uTimeLastRecvFCTelemetry = 0;


   memset( &(g_VehiclesRuntimeInfo[0].headerRubyTelemetryExtended), 0, sizeof(t_packet_header_ruby_telemetry_extended_v2));
   memset( &(g_VehiclesRuntimeInfo[0].headerRubyTelemetryExtraInfo), 0, sizeof(t_packet_header_ruby_telemetry_extended_extra_info));
   memset( &(g_VehiclesRuntimeInfo[0].headerRubyTelemetryExtraInfoRetransmissions), 0, sizeof(t_packet_header_ruby_telemetry_extended_extra_info_retransmissions));


   log_line("[Event]: Handing event BeforePairing. Done.");
}
...
void onEventPairingStopped()
{
   ...
   osd_warnings_reset();
   warnings_remove_all();
   log_line("[Event]: Handing event Pairing Stopped. Done.");
}
...
void onEventArmed()
{
   ...
   log_line("[Event]: Handing event OnArmed. Done.");
}
...
void onEventDisarmed()
{
   ...
   if ( NULL != p && p->iStopVideoRecOnDisarm )
      ruby_stop_recording();
   log_line("[Event]: Handing event OnDisarmed. Done.");
}

bool onEventReceivedModelSettings(u8* pBuffer, int length, bool bUnsolicited)
{
   log_line("[Event]: Handing event OnReceivedModelSettings (%d bytes, expected: %s)...", length, (bUnsolicited?"no":"yes"));
   ...
   // Remove relay flags from radio links flags for vehicles version 7.2 or older (build 45)
   bool bRemoveRelayFlags = false;
   if ( (modelTemp.sw_version>>16) < 45 )
      bRemoveRelayFlags = true;

   if ( bRemoveRelayFlags )
   {
      log_line("Received model settings for vehicle version 7.2 or older. Remove relay flags.");
   ...
   if ( g_pCurrentModel->radioLinksParams.links_count != modelTemp.radioLinksParams.links_count )
      bRadioChanged = true;
   if ( g_pCurrentModel->radioInterfacesParams.interfaces_count != modelTemp.radioInterfacesParams.interfaces_count )
      bRadioChanged = true;
   for( int i=0; i<modelTemp.radioLinksParams.links_count; i++ )
   {
      char szBuff[128];
      ...
      log_line("Received vechile info: radio link %d: %s, capabilities flags: %s, radio flags: %s", i+1, str_format_frequency(modelTemp.radioLinksParams.link_frequency[i]), szBuffC, szBuff);
   }
   ...
   if ( bRemoveRelayFlags )
   {
      log_line("Received model settings for vehicle version 7.2 or older. Remove relay flags on the local stored model.");
      for( int i=0; i<g_pCurrentModel->radioLinksParams.links_count; i++ )
   ...
   log_line("[Event]: Handing of event OnReceivedModelSettings complete.");
   return true;
}

void onEventRelayModeChanged()
{
   log_line("[Event]: Handing event OnRelayModeChanged...");
   log_line("[Event] New relay mode: %d.", g_pCurrentModel->relay_params.uCurrentRelayMode);
   log_line("[Event]: Handing of event OnRelayModeChanged complete.");  
}