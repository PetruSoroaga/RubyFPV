...
void MenuVehicleVideoEncodings::sendVideoLinkProfile()
{
   ...
   if ( m_pItemsSelect[3]->getSelectedIndex() == 0 )
      pProfile->encoding_extra_flags = pProfile->encoding_extra_flags & (~ (ENCODING_EXTRA_FLAG_ENABLE_ADAPTIVE_VIDEO_LINK_PARAMS | ENCODING_EXTRA_FLAG_USE_MEDIUM_ADAPTIVE_VIDEO) );
   else
   {
      if ( m_pItemsSelect[3]->getSelectedIndex() == 1 )
      {
         ...
         pProfile->encoding_extra_flags = pProfile->encoding_extra_flags | ENCODING_EXTRA_FLAG_USE_MEDIUM_ADAPTIVE_VIDEO;
      }
      else
         pProfile->encoding_extra_flags = pProfile->encoding_extra_flags | ENCODING_EXTRA_FLAG_ENABLE_ADAPTIVE_VIDEO_LINK_PARAMS;
   }
   ...
   pProfile->encoding_extra_flags &= ~(ENCODING_EXTRA_FLAG_ENABLE_VIDEO_ADAPTIVE_QUANTIZATION | ENCODING_EXTRA_FLAG_VIDEO_ADAPTIVE_QUANTIZATION_STRENGTH);
   ...
   if ( 1 == m_pItemsSelect[15]->getSelectedIndex() )
      pProfile->encoding_extra_flags |= ENCODING_EXTRA_FLAG_VIDEO_ADAPTIVE_QUANTIZATION_STRENGTH;
   ...
   log_line("Adaptive video quantization: %s, %s",
    ...
    (pProfile->encoding_extra_flags & ENCODING_EXTRA_FLAG_VIDEO_ADAPTIVE_QUANTIZATION_STRENGTH)?"Strength: High":"Strength: Low");
