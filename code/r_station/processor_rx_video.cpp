...
bool ProcessorRxVideo::init()
{
   ...
   log_line("[VideoRX] Initialize video processor Rx instance number %d.", m_iInstanceIndex+1);
...
bool ProcessorRxVideo::uninit()
{
   ...
   log_line("[VideoRX] Uninitialize video processor Rx instance number %d.", m_iInstanceIndex+1);
...
u32 s_uLastReceivedVideoVehicleId = 0;
...
u32 s_uTimeLastVideoChanged = 0;

u32 process_data_rx_video_get_last_time_video_changed()
{
   return s_uTimeLastVideoChanged;
}
...
void _rx_video_reset_receive_state()
{
   ...
   s_uTimeLastVideoChanged = g_TimeNow;
...
void _push_first_block_out()
{
   ...
   if ( s_pRXBlocksStack[0]->received_data_packets < s_pRXBlocksStack[0]->data_packets )
   {
      if ( s_pRXBlocksStack[0]->received_data_packets + s_pRXBlocksStack[0]->received_fec_packets >= s_pRXBlocksStack[0]->data_packets )
      {
         ...
         {
            if ( g_ControllerVehiclesAdaptiveVideoInfo.vehicles[i].uVehicleId != s_uLastReceivedVideoVehicleId )
               continue;
            g_ControllerVehiclesAdaptiveVideoInfo.vehicles[i].uIntervalsOuputRecontructedVideoPackets[g_ControllerVehiclesAdaptiveVideoInfo.vehicles[i].uCurrentIntervalIndex]++;  
         }
   ...
   else
   {
      ...
      for( int i=0; i<MAX_CONCURENT_VEHICLES; i++ )
      {
         if ( g_ControllerVehiclesAdaptiveVideoInfo.vehicles[i].uVehicleId != s_uLastReceivedVideoVehicleId )
            continue;
         g_ControllerVehiclesAdaptiveVideoInfo.vehicles[i].uIntervalsOuputCleanVideoPackets[g_ControllerVehiclesAdaptiveVideoInfo.vehicles[i].uCurrentIntervalIndex]++;
      }
   }
...
void _push_incomplete_blocks_out(int countToPush, bool bReasonTooOld)
{
   ...
   for( int i=0; i<countToPush; i++ )
   {
      ...
      if ( s_pRXBlocksStack[i]->received_data_packets >= s_pRXBlocksStack[i]->data_packets )
      {
         for( int iv=0; iv<MAX_CONCURENT_VEHICLES; iv++ )
         {
            if ( g_ControllerVehiclesAdaptiveVideoInfo.vehicles[iv].uVehicleId != s_uLastReceivedVideoVehicleId )
               continue;
            g_ControllerVehiclesAdaptiveVideoInfo.vehicles[iv].uIntervalsOuputCleanVideoPackets[g_ControllerVehiclesAdaptiveVideoInfo.vehicles[iv].uCurrentIntervalIndex]++;  
         }
      }
           ...
           (s_pRXBlocksStack[i]->received_data_packets + s_pRXBlocksStack[i]->received_fec_packets >= s_pRXBlocksStack[i]->data_packets) )
      {
         ...
         for( int iv=0; iv<MAX_CONCURENT_VEHICLES; iv++ )
         {
            if ( g_ControllerVehiclesAdaptiveVideoInfo.vehicles[iv].uVehicleId != s_uLastReceivedVideoVehicleId )
               continue;
            g_ControllerVehiclesAdaptiveVideoInfo.vehicles[iv].uIntervalsOuputRecontructedVideoPackets[g_ControllerVehiclesAdaptiveVideoInfo.vehicles[iv].uCurrentIntervalIndex]++;  
         }
      }
...
void _check_and_request_missing_packets()
{
   ...
   for( int iv=0; iv<MAX_CONCURENT_VEHICLES; iv++ )
   {
      if ( g_ControllerVehiclesAdaptiveVideoInfo.vehicles[iv].uVehicleId != s_uLastReceivedVideoVehicleId )
         continue;
      g_ControllerVehiclesAdaptiveVideoInfo.vehicles[iv].uIntervalsRequestedRetransmissions[g_ControllerVehiclesAdaptiveVideoInfo.vehicles[iv].uCurrentIntervalIndex] += totalCountRequestedNew;
      g_ControllerVehiclesAdaptiveVideoInfo.vehicles[iv].uIntervalsRetriedRetransmissions[g_ControllerVehiclesAdaptiveVideoInfo.vehicles[iv].uCurrentIntervalIndex] += totalCountReRequested;
   }
   ...
   if ( s_uLastReceivedVideoVehicleId == 0 || s_uLastReceivedVideoVehicleId == MAX_U32 )
   {
      ...
      PH.vehicle_id_dest = s_uLastReceivedVideoVehicleId;
...
int _preprocess_video_packet(int interfaceNb, u8* pBuffer, int length)
{
   ...
   if ( 0 == s_uLastReceivedVideoVehicleId || MAX_U32 == s_uLastReceivedVideoVehicleId )
   {
      s_uLastReceivedVideoVehicleId = pPH->vehicle_id_src;
      log_line("[VideoRX] Started receiving video from vehicle id %u. (Current main vehicle id is %u)", s_uLastReceivedVideoVehicleId, g_pCurrentModel->vehicle_id);
   
   }
   else if ( s_uLastReceivedVideoVehicleId != pPH->vehicle_id_src )
   {
      log_line("[VideoRX] Started receiving video from a different vehicle id %u (old video vehicle id was: %u) (Current main vehicle id is %u).", pPH->vehicle_id_src, s_uLastReceivedVideoVehicleId, g_pCurrentModel->vehicle_id);
      s_uLastReceivedVideoVehicleId = pPH->vehicle_id_src;
   ...
   if ( (s_LastHardEncodingsChangeVideoBlockIndex == MAX_U32) || (pPHVF->video_block_index > s_LastHardEncodingsChangeVideoBlockIndex) )
   {
      ...
      else if ( g_TimeNow > s_TimeLastEncodingsChangedLog + 5000 )
      {
         ...
         s_uTimeLastVideoChanged = g_TimeNow;
         int kf = (int)pPHVF->video_keyframe;
         log_line("[VideoRX] Encodings changed (%u times) on the received stream (at video block index %u):", s_uEncodingsChangeCount, pPHVF->video_block_index);
   ...
   s_VDStatsCache.keyframe = (int)pPHVF->video_keyframe;
   ...
   for( int iv=0; iv<MAX_CONCURENT_VEHICLES; iv++ )
   {
      ...
      if ( g_ControllerVehiclesAdaptiveVideoInfo.vehicles[iv].iLastRequestedLevelShift == -1 )
      ...
      if ( g_ControllerVehiclesAdaptiveVideoInfo.vehicles[iv].iLastRequestedKeyFrame <= 0 )
...
int process_data_rx_video_on_new_packet(int interfaceNb, u8* pBuffer, int length)
{
   ...
   //log_line("DBG rcv %u-%d", pPHVFTemp->video_block_index, pPHVFTemp->video_block_packet_index);
   #endif
...
{   
   ...
   s_uLastReceivedVideoVehicleId = 0;
   s_LastHardEncodingsChangeVideoBlockIndex = tmp1;
   s_LastVideoResolutionChangeVideoBlockIndex = tmp2;
}
